...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `onmyagent CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "onmyagent-file-session-"));
...
try {
  await waitFor(`${onmyagentUrl}/health`);

  const workspaces = await fetchJson(`${onmyagentUrl}/workspaces`, {
  ...
  assert.equal(diskContent, "updated by onmyagent cli\n");
  ...
  console.log(JSON.stringify({ ok: true, onmyagentUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
