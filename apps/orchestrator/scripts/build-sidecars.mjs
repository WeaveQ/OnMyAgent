...
if (!orchestratorVersion) {
  throw new Error(
    "onmyagent-orchestrator version missing in apps/orchestrator/package.json",
  );
}
...
if (!serverVersion) {
  throw new Error(
    "onmyagent-server version missing in apps/server/package.json",
  );
}
...
run("pnpm", ["--filter", "onmyagent-server", "build:bin:all"], repoRoot);
...
const entries = {
  "onmyagent-server": { version: serverVersion, targets: {} },
};

for (const target of targets) {
  const ext = target.id.startsWith("windows") ? ".exe" : "";
  const serverSrc = join(serverDir, `onmyagent-server-${target.bun}${ext}`);
  if (!existsSync(serverSrc)) {
    throw new Error(`Missing onmyagent-server binary at ${serverSrc}`);
  }
  const serverDest = join(outdir, `onmyagent-server-${target.id}${ext}`);
  copyFileSync(serverSrc, serverDest);

  entries["onmyagent-server"].targets[target.id] = {
...
writeFileSync(
  join(outdir, "onmyagent-orchestrator-sidecars.json"),
