...
if (!version) {
  throw new Error("onmyagent-orchestrator version missing in apps/orchestrator/package.json")
}
...
function platformPkgName(target) {
  ...
  return `onmyagent-orchestrator-${platform}-${arch}`
}
...
for (const target of targets) {
  ...
  const src = join(root, "dist", "bin", `onmyagent-${target.bun}${ext}`)
  if (!existsSync(src)) {
    throw new Error(`Missing onmyagent binary at ${src}. Run: pnpm --filter onmyagent-orchestrator build:bin:all`)
  }
  ...
  const dest = join(bindir, `onmyagent${ext}`)
  ...
  writeJson(join(dir, "package.json"), {
    name,
    version,
    description: "Platform binary for onmyagent-orchestrator",
    license: "Apache-2.0",
    os: [target.os],
    cpu: [target.cpu],
    bin: {
      onmyagent: `./bin/onmyagent${ext}`,
      "onmyagent-orchestrator": `./bin/onmyagent${ext}`,
    },
    files: ["bin"],
  })
...
const meta = join(outroot, "onmyagent-orchestrator")
mkdirSync(join(meta, "bin"), { recursive: true })

const wrapperSrc = join(root, "bin", "onmyagent")
if (!existsSync(wrapperSrc)) {
  throw new Error(`Missing wrapper at ${wrapperSrc}`)
}
copyFileSync(wrapperSrc, join(meta, "bin", "onmyagent"))
chmodSync(join(meta, "bin", "onmyagent"), 0o755)
...
writeJson(join(meta, "package.json"), {
  name: "onmyagent-orchestrator",
  version,
  description: "OnMyAgent host orchestrator for opencode + OnMyAgent server",
  license: "Apache-2.0",
  bin: {
    onmyagent: "./bin/onmyagent",
    "onmyagent-orchestrator": "./bin/onmyagent",
  },
  scripts: {
...
published.push({ name: "onmyagent-orchestrator", dir: meta })
