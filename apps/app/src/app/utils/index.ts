...
export function isElectronRuntime() {
  return typeof window !== "undefined" && (window as Window).__ONMYAGENT_ELECTRON__ != null;
}
...
const STARTUP_PREF_KEY = "onmyagent.startupPref";
const LEGACY_PREF_KEY = "onmyagent.modePref";
const LEGACY_PREF_KEY_ALT = "onmyagent_mode_pref";
...
export function getWorkspaceTaskLoadErrorDisplay(workspace: WorkspaceInfo, error?: string | null) {
  ...
  const fallbackTitle = raw || t("app.error_load_tasks");
  if (!raw || !isSandboxWorkspace(workspace)) {
    return {
      tone: "error" as const,
      label: t("common.error"),
      message: raw && workspace.workspaceType === "remote" ? raw : t("app.error_load_tasks"),
      title: fallbackTitle,
    };
  }
  ...
  const host = `${workspace.baseUrl ?? ""} ${workspace.onmyagentHostUrl ?? ""}`.toLowerCase();
  ...
  if (!hasDockerHint && !(localHost && hasNetworkHint)) {
    return {
      tone: "error" as const,
      label: t("common.error"),
      message: t("app.error_load_tasks"),
      title: fallbackTitle,
    };
  }

  const message = t("workspace.sandbox_offline_docker_hint");
  return {
...
const TRUNCATED_SYSTEM_PATH_PATTERNS = [
  /com\.[^/]+\.(onmyagent|opencode)/i, // macOS app bundle identifiers
  /\.onmyagent\.dev\//i, // OnMyAgent dev paths
...
export function summarizeStep(part: Part): { title: string; detail?: string; isSkill?: boolean; skillName?: string; toolCategory?: string; status?: string } {
  ...
  return { title: t("session.step"), toolCategory: "tool" };
}
