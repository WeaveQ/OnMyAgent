// Legacy migration snapshot ingestion (Electron side only).
//
// The previous desktop shell (Tauri) used to export localStorage keys into
// a JSON file. Electron reads that file on first launch, hydrates
// localStorage for keys that are still empty, then marks the file as
// acknowledged.
//
// The previous shell's write function has been removed. This module only
...
export const MIGRATION_KEY_PATTERNS: Array<RegExp> = [
  /^onmyagent\.react\.activeWorkspace$/,
  /^onmyagent\.react\.sessionByWorkspace$/,
  /^onmyagent\.server\.list$/,
  /^onmyagent\.server\.active$/,
  /^onmyagent\.server\.urlOverride$/,
  /^onmyagent\.server\.token$/,
];
...
export const MIGRATION_DEFER_KEY = "onmyagent.migration.deferredUntil";
