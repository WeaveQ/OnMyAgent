...
const resolveAuthHeader = (auth?: OpencodeAuth) => {
  if (auth?.mode === "onmyagent" && auth.token) {
 ...
 * message streams, event subscriptions). In the desktop Electron app, the
 * IPC bridge's fetch can block until the entire body is delivered, so
 ...
 * CORS is already wide open on the onmyagent/opencode stack, so there's no
 * reason to route them through the IPC bridge.
 */
...
export function createClient(baseUrl: string, directory?: string, auth?: OpencodeAuth) {
  ...
  sessionOverrides.list = (parameters?: SessionListParameters, options?: { throwOnError?: boolean }) => {
    if (!onmyagentMount || !onmyagentSessionClient) {
