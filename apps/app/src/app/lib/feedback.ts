const ENV_FEEDBACK_URL = String(import.meta.env.VITE_ONMYAGENT_FEEDBACK_URL ?? "").trim();
const ENV_APP_VERSION = String(import.meta.env.VITE_ONMYAGENT_APP_VERSION ?? "").trim();

export const DEFAULT_FEEDBACK_URL =
  ENV_FEEDBACK_URL || "https://onmyagentlabs.com/feedback";

type FeedbackUrlOptions = {
  entrypoint: string;
  deployment?: string | null;
  appVersion?: string | null;
  onmyagentServerVersion?: string | null;
...
export function buildFeedbackUrl(options: FeedbackUrlOptions): string {
  ...
  url.searchParams.set("source", "onmyagent-app");
  ...
  const entries = {
    ...
    onmyagentServerVersion: options.onmyagentServerVersion?.trim() ?? "",
