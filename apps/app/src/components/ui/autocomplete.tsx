...
export function AutocompleteInput({
  ...
  const { className: clearClassName, ...clearRestProps } = clearProps ?? {};
  ...
  return (
    ...
    >
      ...
      {showTrigger && (
        <AutocompleteTrigger
          className={cn("has-[+[data-slot=autocomplete-clear]]:hidden", triggerClassName)}
          {...triggerRestProps}
        >
      ...
      {showClear && (
        <AutocompleteClear
          className={cn("has-[+[data-slot=autocomplete-clear]]:hidden", clearClassName)}
          {...clearRestProps}
        >
          <XIcon />
        </AutocompleteClear>
  ...
  return (
    <AutocompletePrimitive.Portal {...portalProps}>
      ...
      >
        <span
          className={cn(
            "relative flex max-h-full min-w-(--anchor-width) max-w-(--available-width) origin-(--transform-origin) rounded-lg border bg-popover not-dark:bg-clip-padding/5 transition-[scale,opacity] before:pointer-events-none before:absolute before:inset-0 before:rounded-lg",
            className,
          )}
        >
...
export function AutocompleteItem({
  ...
  return (
    <AutocompletePrimitive.Item
      className={cn(
        "flex min-h-8 cursor-pointer select-none items-center rounded-sm px-2 py-1 text-base outline-none data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:min-h-7 sm:text-sm",
        className,
      )}
...
export function AutocompleteTrigger({
  ...
  return (
    <AutocompletePrimitive.Trigger
      className={cn(autocompleteActionClass, className)}
