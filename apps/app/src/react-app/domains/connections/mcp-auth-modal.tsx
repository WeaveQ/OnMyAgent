...
export function McpAuthModal(props: McpAuthModalProps) {
  ...
  useEffect(() => {
    ...
    if (!props.open || !awaitingReload || props.reloadBlocked || !props.onReloadEngine || !entry || reloadAuthRunRef.current) {
      return;
    }
    ...
    >
      <DialogContent className="flex max-h-[90vh] min-h-0 w-full max-w-lg flex-col overflow-hidden sm:max-w-lg">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto">
            ...
            <div className="space-y-4 rounded-xl border border-dls-mist bg-dls-surface px-5 py-6 text-center">
              ...
              <div className="space-y-2">
                ...
                <Button
                  type="button"
                  variant="link"
                  className="h-auto p-0 text-xs text-dls-secondary hover:text-dls-secondary"
                  onClick={handleRetry}
                >
              ...
              {props.reloadBlocked && (props.activeSessions?.length ?? 0) > 0 ? (
                <div className="space-y-2 text-left">
                  {(props.activeSessions ?? []).map((session) => (
                    <NoticeBox key={session.id} tone="warning" className="flex items-center justify-between gap-3">
                      ...
                      <Button
                        type="button"
                        variant="link"
                        className={mcpAuthStateClass.warningLink}
          ...
          {error ? (
            <div className={mcpAuthStateClass.errorPanel}>
              <p className={mcpAuthStateClass.errorText}>{error}</p>
              ...
              {isInvalidRefreshToken() ? (
                <div className="space-y-2 pt-2">
                  <p className={mcpAuthStateClass.errorHint}>{t("mcp.auth.invalid_refresh_token")}</p>
                  {!props.isRemoteWorkspace ? (
                    isDesktopRuntime() ? (
                      ...
                      <div className={mcpAuthStateClass.errorHint}>
                        {t("mcp.auth.reauth_cli_hint", { server: serverName })}
                      </div>
                    )
                  ) : (
                    <div className={mcpAuthStateClass.errorHint}>{t("mcp.auth.reauth_remote_hint")}</div>
                  )}
                  {cliAuthResult ? <div className={mcpAuthStateClass.errorHint}>{cliAuthResult}</div> : null}
                </div>
              ) : null}
            </div>
          ) : null}
            ...
            <div className="space-y-3 rounded-xl border border-dls-mist bg-dls-surface p-4">
              ...
              <div className="flex items-center gap-3 rounded-xl border border-dls-mist bg-dls-surface-muted px-3 py-2">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate font-mono text-xs text-dls-secondary">{authorizationUrl}</div>
                </div>
              ...
              />
              <div className="text-xs text-dls-secondary">{t("mcp.auth.port_forward_hint")}</div>
              ...
              <NoticeBox size="comfortable">
                <div className="space-y-3">
                  ...
                  <Button
                    type="button"
                    variant="link"
                    className="h-auto justify-start p-0 text-left text-xs text-dls-secondary hover:text-dls-secondary"
                    onClick={handleRetry}
                  >
                    {t("mcp.auth.reopen_browser_link")}
                  </Button>
