...
function readUiControlBridgeInfo(value: unknown) {
...
export function createConnectionsStore(options: {
  ...
  const resolveWritableOnMyAgentTarget = async () => {
    ...
    if (!onmyagentWorkspaceId && onmyagentClient && onmyagentSnapshot.onmyagentServerStatus === "connected") {
    ...
    const canUseOnMyAgentServer =
      ...
      onmyagentClient &&
      onmyagentWorkspaceId &&
      onmyagentCapabilities?.mcp?.write;
  ...
  const listMcpFromOnMyAgentServer = async (projectDir: string) => {
    ...
    const canTryOnMyAgentServer =
      ...
      onmyagentSnapshot.onmyagentServerCapabilities?.mcp?.read !== false;
    ...
    try {
      ...
      if (!fallbackOnError) {
        throw error instanceof Error
          ? error
          : new Error("Computer Use helper app is unavailable. Restart OnMyAgent or reinstall the app.");
      }
  ...
  const resolveLocalMcpCommand = async (entry: McpDirectoryInfo) => {
    ...
    if (mcpResource?.localCommandRef === "onmyagent.computerUseMcp") {
  ...
  async function refreshMcpServers() {
    ...
    if (isRemoteWorkspace) {
      mutateState((current) => ({
        ...current,
        mcpStatus: "OnMyAgent server unavailable. MCP config is read-only.",
        mcpServers: [],
        mcpStatuses: {},
      }));
      return;
  ...
  async function connectMcp(entry: McpDirectoryInfo) {
    ...
    try {
      ...
      if (!resolvedUrl && entry.serverName === "onmyagent-ui") {
        try {
          const bridgeInfo = readUiControlBridgeInfo(
            await window.__ONMYAGENT_ELECTRON__?.invokeDesktop?.("getUiControlBridgeInfo"),
          );
          if (bridgeInfo) {
      ...
      if (entryType === "remote") {
        if (!resolvedUrl) {
          throw new Error("Missing MCP URL. Is the OnMyAgent desktop app running?");
        }
      ...
      if (canUseOnMyAgentServer && onmyagentClient && onmyagentWorkspaceId) {
        ...
        // id that the OnMyAgent server does not expose, producing a confusing
  ...
  async function setMcpEnabled(name: string, enabled: boolean) {
    try {
      ...
      await onmyagentClient.setMcpEnabled(onmyagentWorkspaceId, name, enabled);
