...
const EXTENSION_DISABLED_KEY_PREFIX = "onmyagent.extension.disabled.";
const EXTENSION_ENABLED_KEY_PREFIX = "onmyagent.extension.enabled.";
const EXTENSION_HIDDEN_KEY_PREFIX = "onmyagent.extension.hidden.";
export const ONMYAGENT_EXTENSION_STATE_CHANGED = "onmyagent:extension-state-changed";

export function getExtensionId(entry: McpDirectoryInfo): string {
  return entry.id ?? entry.serverName ?? getMcpServerName(entry);
}

export function isOnMyAgentExtensionEnabled(entry: McpDirectoryInfo): boolean {
...
export function setOnMyAgentExtensionEnabled(entry: McpDirectoryInfo, enabled: boolean) {
  ...
  window.dispatchEvent(new CustomEvent(ONMYAGENT_EXTENSION_STATE_CHANGED, {
    detail: { id, enabled },
  }));
}

export function isOnMyAgentExtensionHidden(entryOrId: McpDirectoryInfo | string): boolean {
...
export function setOnMyAgentExtensionHidden(entryOrId: McpDirectoryInfo | string, hidden: boolean) {
  const id = typeof entryOrId === "string" ? entryOrId : getExtensionId(entryOrId);
  if (typeof window === "undefined") return;
