...
export function CustomConnectorDialog(props: CustomConnectorDialogProps) {
  ...
  const refreshStatuses = useCallback(async () => {
    ...
    try {
      const raw = unwrap(
        await mcpClient.mcp.status(directory ? { directory } : undefined),
      ) as McpStatusMap;
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex shrink-0 items-center gap-1">
                                ...
                                <Switch
                                  ...
                                  checked={toggledOn}
                                  disabled={busy}
                                  onCheckedChange={(next) => void handleToggle(entry.name, next)}
                                  aria-label={entry.name}
                                />
                              </div>
                            </div>
                            {open && (errorText || busy) ? (
                              <div className="mt-2.5 rounded-lg border border-dls-status-danger-border bg-dls-status-danger-soft px-3 py-2.5">
                                <p className="whitespace-pre-wrap break-words font-mono text-xs leading-5 text-dls-status-danger-fg">
                                  {busy ? t("plugins.custom_connector_reconnecting") : errorText}
                                </p>
...
export function CustomConnectorEntryButton(props: { onClick: () => void; className?: string }) {
  ...
  return (
    ...
    >
      <KoboyoIcon src={KOBOYO_BADGE_PLUS} size={14} className="text-dls-secondary bg-current" />
