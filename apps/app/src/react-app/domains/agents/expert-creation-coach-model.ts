...
export function applyExpertCoachProposal(
  ...
  _skills: readonly AgentSkillItem[],
): AgentWizardDraft {
  return {
    ...draft,
    name: proposal.name,
...
export function buildExpertCoachSystemPrompt(
  ...
  return [
    ...
    "Help the user clarify the expert's purpose, audience, boundaries, workflow, tone, and memory.",
    ...
    "When there is enough information, return a complete proposal with an empty `skillIds` array.",
    "Never select, add, remove, or change skills. Skills are configured only through the user's explicit controls in the skills tab.",
