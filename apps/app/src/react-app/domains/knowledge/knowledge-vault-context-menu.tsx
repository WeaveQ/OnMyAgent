...
export function KnowledgeVaultContextMenu(props: KnowledgeVaultContextMenuProps) {
  ...
  const panelClass = "min-w-40 p-1 bg-dls-surface text-dls-text ring-1 ring-dls-border";
          ...
          <>
            ...
            {canSearchFolder ? (
              <ContextMenuItem
                className={itemClass}
                onClick={() =>
                  props.onSearchInFolder(props.target.kind === "dir" ? props.target.path : folder)
                }
              >
            ...
            <ContextMenuSub>
              ...
              <ContextMenuSubContent className={panelClass}>
                <ContextMenuItem
                  className={itemClass}
                  onClick={() => props.onCopyPath(props.target, "rel")}
                >
                  {t("knowledge.copy_rel_path")}
                </ContextMenuItem>
                <ContextMenuItem
                  className={itemClass}
                  onClick={() => props.onCopyPath(props.target, "abs")}
                >
