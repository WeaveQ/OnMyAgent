...
import { isOnMyAgentExtensionEnabled, setOnMyAgentExtensionEnabled } from "../../../shared";
import {
...
import { ComposerToolMenuMine, type ComposerToolMenuMineProps } from "./composer-tool-menu-mine";
...
export type ComposerToolMenuProps = {
  ...
  applyManagedConnectorPrompt: (connector: ManagedDesktopConnectorItem, prompt: string) => void;
  ...
  setSelectedManagedConnector: (connector: ManagedDesktopConnectorItem | null) => void;
          ...
          {toolMenuSection === "mine" ? null : (
            <div
              className={cn(
                ...
                toolMenuSection === "skills" || toolMenuSection === "mcps" ? "pt-0" : "pt-1.5",
                    ...
                    return (
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-2">
                          ...
                          <span className="truncate text-sm text-dls-text">{template.label}</span>
                        </span>
                    ...
                    <p className="whitespace-nowrap text-sm leading-5 text-dls-secondary">
                      {
                        (
                          modeOptions.find(
                            (option) => option.key === (selectedModeKey ?? "craft"),
                          ) ?? modeOptions[0]
                        )?.description
                      }
                    </p>
                    ...
                    {modeOptions.map((option) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {checked ? <Check className="size-3.5 shrink-0 text-dls-text" /> : null}
                        </MenuRowButton>
                      );
                    })}
                  </div>
                        ...
                        const rowBody = (
                          <div className="group/skill flex min-w-0 items-stretch gap-0.5 rounded-xl hover:bg-dls-surface-muted/70">
                            ...
                            <button
                              type="button"
                              className={cn(
                                "inline-flex size-8 shrink-0 !w-8 items-center justify-center rounded-lg outline-none transition-colors focus-visible:ring-2 focus-visible:ring-dls-focus focus-visible:ring-offset-2",
                                isPinned
                                  ? "text-dls-accent hover:bg-dls-surface-muted"
                                  : "text-dls-secondary hover:bg-dls-surface-muted hover:text-dls-text",
                              )}
                              title={isPinned ? t("composer.unpin_skill") : t("composer.pin_skill")}
                              aria-label={
                                isPinned ? t("composer.unpin_skill") : t("composer.pin_skill")
                              }
                            ...
                            const rowBody = (
                              <div
                                className={cn(
                                  ...
                                  selectedManagedConnector?.id === connector.id && "bg-dls-hover",
                                )}
                                onMouseEnter={() => {
                                  setSelectedComposerExtension(null);
                                  setSelectedManagedConnector(hasPrompts ? connector : null);
                                }}
                              >
                                <IconTile
                                  size="sm"
                                  ...
                                  tone="surface"
                                  border
                                  className={cn(extensionIconTileClassName, "overflow-hidden")}
                                >
                                  <img
            ...
            <div className="grid gap-0.5">
              {selectedManagedConnector.tryPrompts.map((prompt) => (
                <MenuRowButton
                  ...
                  density="compact"
                  className="gap-2"
                  onClick={() => applyManagedConnectorPrompt(selectedManagedConnector, prompt)}
                >
                  <MessageCircle className="mt-0.5 size-3.5 shrink-0 text-dls-secondary" />
                  <span className="line-clamp-2 text-sm leading-5 text-dls-text">{prompt}</span>
                </MenuRowButton>
