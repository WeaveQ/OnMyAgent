...
当 OnMyAgent 在工作区使用 **`.onmyagent/teams/<teamId>/inboxes/*.json`** 时，福帮手多智能体（team-lead / reviewer / writer）的**任务状态**宜与收件箱字段对齐，避免「对话里并行、磁盘未同步」。
...
1. **探测**：若存在 `.onmyagent/teams/**/inboxes/team-lead.json`，首响提示「检测到团队编排目录」。
2. **写入**：仅由 **team-lead** 或明确授权脚本写入 inbox；子智能体优先落盘 `.fbs/` 再通知。
3. **冲突**：以 `.fbs` 与 `onmyagent-resume.json` 为准；inbox 为协调层。
