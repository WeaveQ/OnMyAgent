...
const AUTH_DIR = path.join(require('os').homedir(), '.onmyagent', 'credentials', 'meituan-living-deals-assistant');
  ...
  try {
    ...
    const result = spawnSync(PYTHON, cmdArgs, {
      ...
      env: Object.assign({}, process.env, sslEnv, {
        ONMYAGENT_AUTH_FILE: path.join(AUTH_DIR, 'token.json'),
        NODE_OPTIONS: ''
      })
    });
...
commands.init = function () {
  ...
  var clientType = (function () {
    var envType = (process.env.ONMYAGENT_CLIENT_TYPE || '').toLowerCase();
...
commands.qrcode = function (argv) {
  ...
  const body = { originalUrl: url, clientSource: 'coupon-fusion-onmyagent' };
...
commands.search = function (argv) {
  ...
  const body = {
    ...
    clientSource: 'coupon-fusion-onmyagent',
    userParamDTO: {
      lat: args['lat'],
      lng: args['lng'],
      token: token,
