...
.DESCRIPTION
  递归扫描 BookRoot 下的 Markdown 文件（含 deliverables；排除 .fbs/.onmyagent/node_modules 等），输出以下机器可检项：
  - S2 冗余修饰词密度
  - S4 连接词密度
  - S5 buzzword 命中数
  - S6 破折号密度
...
$markdownFiles = Get-ChildItem -Path $bookRootPath -Recurse -File |
    Where-Object {
        ...
        $_.FullName -notmatch '[\\/](node_modules|\.git|\.fbs|\.onmyagent|qc-output|releases|\.codebuddy|\.codebuddy-plugin)[\\/]'
