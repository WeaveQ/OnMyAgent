...
> OnMyAgent 专家市场上架的 **Agent 型** 单专家。**9 大内置技能加持**，从"想出门"到"花得值"全流程托底的 AI 旅行管家。
...
| 11 | 优惠券赋能 | 餐饮/酒店/门票/外卖美团红包 | `meituan-coupon-onmyagent` |
...
│   └── meituan-coupon-onmyagent/    # 美团红包助手（Python）
├── avatars/
...
| "帮我领美团红包"/"手头紧想省点" | `meituan-coupon-onmyagent` | 不需要（需用户登录美团） |
...
Step 4 风险贴士（预约 / 物品 / 避坑 / App / 飞行气象）
    ...
    ↓
Step 5 省钱加成 → meituan-coupon-onmyagent
    ↓
...
| 运行时 | 相关技能 |
|-------|---------|
| `python3` | 12306-train-assistant、flight-tracker、aviation-weather、meituan-coupon-onmyagent |
...
- **美团登录**：严格按 `meituan-coupon-onmyagent/SKILL.md` 执行；登录由用户主动发起；不输出完整 token；手机号脱敏
