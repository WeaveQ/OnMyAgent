# OnMyAgent 审核 / 集成检查清单（FBS-BookWriter v2.1.2）
...
- [ ] `dist/fbs-bookwriter-v212-onmyagent.zip` 可生成且无报错  
- [ ] `SKILL.md`、`onmyagent/channel-manifest.json`、`_plugin_meta.json` 版本号一致  
...
- [ ] 已读 [`references/06-plugin/onmyagent-host-integration.md`](../references/06-plugin/onmyagent-host-integration.md)  

## C. 体验契约（抽样）

- [ ] `npm run pack:skill-gates` 通过（或等价 CI 任务）  
- [ ] `node scripts/ux-flow-guard.mjs --skill-root . --book-root . --enforce` 通过  
- [ ] `intake-router --json` 含 `firstResponseContext`（首响环境三元组）；`session-exit` 退出后存在 `书稿根/.onmyagent/memory/当日.md` 叙事镜像（或显式 `--no-onmyagent-mirror`）  
...
- [ ] **`historicalBookShortcuts`**：`~/.onmyagent/fbs-book-projects.json` 中与**当前书稿不同**的最近登记项（最多 2 条），用于首屏「接着写」分流  
- [ ] **`deliveryAndPreview`**：`deliverables` 路径说明 + 是否 `presentationBridgeSupported`；详见 [`references/05-ops/onmyagent-delivery-tier2.md`](../references/05-ops/onmyagent-delivery-tier2.md)  
- [ ] **`capabilityRefresh`**：当 `~/.onmyagent/.skills-marketplace-version` 新于 `.fbs/host-capability.json` 探测时间时，提示刷新；并出现可选「刷新宿主能力快照」动作  

### E2 检索与交付策略（B1/B2）

- [ ] **`searchStrategyHints`**：按已探测到的市场能力生成 **Agent 内**检索路由提示（中文/学术/深度等），**不向用户念技能 ID**  
- [ ] Tier2 交付顺序与文档：[`onmyagent-delivery-tier2.md`](../references/05-ops/onmyagent-delivery-tier2.md)  
...
- [ ] 类型与示例：[`references/06-plugin/onmyagent-host-integration.md`](../references/06-plugin/onmyagent-host-integration.md) §1.0.1–§1.0.2  
...
- [ ] **执行与安全边界**：向终端用户说明见 [`onmyagent-host-integration.md`](../references/06-plugin/onmyagent-host-integration.md)「执行与安全边界」；**勿**公网暴露高权限宿主入口  
