 ...
 * - 统一写入 .fbs/onmyagent-resume.json 与 .fbs/smart-memory/session-resume-brief.md
 * - 为下次输入“福帮手”提供可恢复入口
 */
...
import { appendOnMyAgentMemoryMirror } from './lib/onmyagent-workspace-memory.mjs';
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (token === '--no-onmyagent-mirror') args.noOnMyAgentMirror = true;
...
行为：
  1. 写入 .fbs/onmyagent-resume.json
  2. 写入 .fbs/smart-memory/session-resume-brief.md
  3. 返回“已记录当前状态，下次输入『福帮手』可继续”
  4. 默认向 书稿根/.onmyagent/memory/当日.md 追加简短镜像（叙事层，与 .fbs 真值并行）；加 --no-onmyagent-mirror 可关闭
`);
}

export async function handleSessionExit({ bookRoot, note = '', quiet = false, mirrorOnMyAgentMemory = true } = {}) {
  ...
  let onmyagentMirrorPath = null;
  if (mirrorOnMyAgentMemory && memoryResult.briefPath && fs.existsSync(memoryResult.briefPath)) {
    const briefHead = fs
      .readFileSync(memoryResult.briefPath, 'utf8')
      .split('\n')
      .slice(0, 14)
      .join('\n');
    const mirror = appendOnMyAgentMemoryMirror({
      bookRoot: resolvedBookRoot,
      title: 'FBS 退出摘要',
      lines: [
    ...
    onmyagentMirrorPath = mirror.path;
  }
  ...
  if (modN > 0) {
    userMessage += `\n📌 本次可核对变更路径 ${modN} 条（已写入 .fbs/onmyagent-resume.json → modifiedFiles）。`;
  }
  ...
  const out = {
    ...
    onmyagentMirrorPath,
    files: {
      resumeCard: path.join(fbsDir, 'onmyagent-resume.json'),
    ...
    userSummary,
    userMessage,
    /** OnMyAgent 复盘 2026-04-16：本次会话变更文件摘要（git 可用时） */
    sessionChangeSummary,
    /** 供宿主/Agent 展示：退出前软确认（2026-04-13 OnMyAgent 实测 P0-3） */
    agentGuidance: {
      beforeExit:
...
async function main() {
  ...
  try {
    const result = await handleSessionExit({
      ...
      mirrorOnMyAgentMemory: !args.noOnMyAgentMirror,
    });
