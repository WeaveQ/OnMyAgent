...
- `fbs-runtime-hints.json` → `compliance` 与 `scriptBridge` 段；**宿主展示**另见 **`hostPresentation`**（用户可见以 `userFacingOneLiner` + ≤3 主选项为准；可选 `healthUserHint`、`runtime.healthMatrix` 路径见同文件 `hostPresentation.*JsonPath`）；**渠道默认**见 **`channelSessionDefaults`**；**安全原则摘要**见 **`executionSafety`**；**长任务契约**见 **`subTaskContract`**（见 [`../06-plugin/onmyagent-host-integration.md`](../06-plugin/onmyagent-host-integration.md) §1.0 与「执行与安全边界」）
...
- `onmyagent/channel-manifest.json` / `codebuddy/channel-manifest.json` → `scriptBridge`（检索前置合同、企微场景包、乐包仍走脚本，**不拆 MCP**）
...
建议在 **S1 定名后**维护与展示一致的书名与体量，供 `onmyagent-session-snapshot` 与恢复卡使用：
...
OnMyAgent 等宿主可提供**系统级记忆**能力（与书稿目录 `.fbs/` 内文件相互独立）。福帮手与之对齐时遵守：
