...
- **首屏（只说「福帮手」时）**：先一句话状态 + **最多 3 个主选项**（写新书 / 接着写 / 质检或整理素材），勿首屏平铺长菜单；需求收齐后优先「一次性汇总确认再执行」— 见 `references/01-core/intake-and-routing.md`（OnMyAgent 实测复盘节）。
...
- **宿主工具盲区（复盘 P0）**：部分宿主上 **`search_file` 不进入** dot-directory（`.fbs/`、`.onmyagent/` 等）。检查 `onmyagent-resume.json`、`esm-state.md`、`session-resume-brief.md` 等 **禁止**依赖 `search_file`；应 **`read_file` 直读已知路径**（`.fbs/…`），或 **`list_dir` 书稿根**确认 `.fbs/` 是否存在。**Found 0 不得**解读为「未初始化 FBS」。详见 [`references/01-core/skill-full-spec.md`](./references/01-core/skill-full-spec.md) §2.4。
...
1. 强制入口（首次 / bookRoot 变更 / 仅激活时都先执行）：
   ...
   → 默认快速开场；若用户已知需完整在线场景包，再加 `--full`。可按书名关键词检索历史书稿目录：`--search <关键词>`（依赖曾成功退出后登记在 ~/.onmyagent/fbs-book-projects.json 的索引）。
   → 该脚本会自动完成：宿主检测、恢复卡补写、session-resume-brief 补写、首响路由判断

2. 恢复优先（按脚本输出执行，不要先 list_dir 再反推状态）：
   - IF exists(.fbs/onmyagent-resume.json) → 读取恢复卡 → 恢复会话
   - ELSE IF exists(.fbs/chapter-status.md) → 读取章节台账 → 自动补写恢复卡后恢复
   - ELSE → 进入 S0.5 轻量引导

3. 融合顺序：
   - 先读 .fbs/onmyagent-resume.json
...
| 继续写稿 / 接着写 | 读 `.fbs/onmyagent-resume.json` 或 `.fbs/chapter-status.md` | 不重问背景；若当前在 **S0 且素材已达标**，见「继续」一行 |
...
- 源仓库采用 **OnMyAgent / CodeBuddy 双通道**：
  - `.codebuddy-plugin/plugin.json` → `codebuddy/channel-manifest.json`
  - OnMyAgent 审核包 → `onmyagent/channel-manifest.json`
- `pack:onmyagent` 生成 OnMyAgent 审核包，`pack:openclaw` 生成 OpenClaw 技能包，`pack:release` 一次生成 OnMyAgent、CodeBuddy、OpenClaw 三种发布产物。
- 宿主真值统一以 `node scripts/host-capability-detect.mjs --book-root <bookRoot>` 的输出为准；首响的 `intake-router.mjs` 会自动调用该探测。
- Tier1 本地市场能力只在 **OnMyAgent** 可用；**CodeBuddy** 走 Tier2 宿主插件与内置脚本兜底。
...
- 恢复链路统一以 `.fbs/onmyagent-resume.json`、宿主画像桥接与 Smart Memory 为准。
...
- 恢复卡 `.fbs/onmyagent-resume.json` 含 `lastAction`、`nextRecommendations`（与 `nextSuggested` 同义）、`modifiedFiles`（git 可用时），供杜氏类审计要求的「退出可量化、下次可续」

随后再回复用户：**已记录当前状态，下次输入“福帮手”可继续**。只有用户明确说“不保存”，才允许跳过写盘直接退出。

**兜底（宿主未执行脚本时）**：若无法运行 `session-exit`，至少人工更新 `onmyagent-resume.json` 的 `lastAction` 与 `modifiedFiles`（或 `session-resume-brief.md` 中写明本次改动文件），避免再次出现「改了稿但恢复卡为空」。
...
- `onmyagent/channel-manifest.json` 中的 `presentationConsumer` 是**宿主结果展示入口**，负责返回下一步 `hostAction`，不是**自动执行脚本**
...
- **OnMyAgent 宿主最小集成**：[`references/06-plugin/onmyagent-host-integration.md`](./references/06-plugin/onmyagent-host-integration.md)
