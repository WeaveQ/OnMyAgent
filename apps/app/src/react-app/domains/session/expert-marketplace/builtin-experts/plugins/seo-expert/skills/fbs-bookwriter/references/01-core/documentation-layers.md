...
- `references/01-core/onmyagent-agent-briefings.md` - 多智能体委派话术
...
- `onmyagent/channel-manifest.json` - OnMyAgent 通道清单
- `scripts/pack-onmyagent-marketplace.mjs` - OnMyAgent 审核包入口
- `scripts/pack-release.mjs` - 三通道打包入口（OnMyAgent + CodeBuddy + OpenClaw）
...
| 通道 | 主清单 | 说明 |
|------|--------|------|
| OnMyAgent | `onmyagent/channel-manifest.json` | 审核包、Tier1 本地市场能力 |
| CodeBuddy | `.codebuddy-plugin/plugin.json` + `codebuddy/channel-manifest.json` | 插件包、Tier2 宿主插件能力 |

> 源仓库采用 **OnMyAgent / CodeBuddy 双通道分轨**，不是 OnMyAgent-only 单通道。
