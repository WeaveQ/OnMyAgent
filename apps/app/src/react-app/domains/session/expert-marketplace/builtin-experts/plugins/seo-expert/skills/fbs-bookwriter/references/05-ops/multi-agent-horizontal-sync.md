...
> 话术与编排：[`onmyagent-agent-briefings.md`](../01-core/onmyagent-agent-briefings.md) · [`session-protocols.md`](../01-core/session-protocols.md)
...
> 适用范围：OnMyAgent/CodeBuddy 的 Team 场景（审计、巡检、质检）
...
1. **子智能体完整结果必须先落盘**：优先写入 `{bookRoot}/.onmyagent/memory/qc-{taskId}.md`。
   - 裸仓库场景也可用，不依赖 `.fbs/`
...
4. **中断恢复依赖落盘文件**：恢复时先扫描 `.onmyagent/memory/qc-*.md`，跳过已完成范围。
5. **禁止行为**：
   - ✗ 禁止把完整质检报告直接放进子智能体返回值
   - ✗ 禁止把裸仓库任务结果只写进 `.fbs/`
   - ✗ 禁止多个子智能体竞争写同一个结果文件

## 失响、超时与恢复（OnMyAgent 运行态）
...
4. **宿主侧**：若 OnMyAgent 提供任务队列/心跳，应与 `shutdown_request` / 心跳协议（见上文章节）对齐；无队列时退化为 **串行重试**，体验变慢但语义更安全。
