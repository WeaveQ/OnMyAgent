...
/** Markdown 表格「当前状态/当前阶段」列中的 S0–S6（人工维护时优先于 frontmatter，见 OnMyAgent 实测 P0-01） */
...
export async function generateSessionSnapshot({ fbsDir, quiet = false, includeGitChanges = false } = {}) {
  ...
  const outPath = path.join(fbs, 'onmyagent-resume.json');
  ...
  try {
    const writeResult = writeJsonAtomic(fbs, 'onmyagent-resume.json', snapshot, {
      backup: true,
      quiet,
...
// ── CLI 入口 ──
if (process.argv[1] && process.argv[1].endsWith('onmyagent-session-snapshot.mjs')) {
