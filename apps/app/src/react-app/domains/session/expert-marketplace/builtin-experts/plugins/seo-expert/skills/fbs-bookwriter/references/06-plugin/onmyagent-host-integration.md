# OnMyAgent 宿主集成说明（最小契约）

> **版本**：2.1.2  
> **受众**：OnMyAgent 宿主 / 插件加载器实现方  

## 1. 会话启动时必须可调用的脚本（与 `onmyagent/channel-manifest.json` → `entry` 对齐）

| 键 | 路径 | 用途 |
|----|------|------|
| `hostCapability` | `scripts/host-capability-detect.mjs` | 探测 `~/.onmyagent`、Tier1 技能目录、插件；写 `.fbs/host-capability.json` |
| `intakeRouter` | `scripts/intake-router.mjs` | 首响路由、恢复优先、输出 JSON 动作列表 |
| `sessionSnapshot` | `scripts/onmyagent-session-snapshot.mjs` | 生成/刷新恢复卡 |
| `profileBridge` | `scripts/onmyagent-user-profile-bridge.mjs` | 宿主画像 → 开场协议 |
...
> 与两轮 OnMyAgent 实测对齐：**用户面对的是服务，不是开发文档**。
...
- 宿主（企微 / 飞书 / OnMyAgent）宜在**会话级**覆盖：未覆盖时采用 `intake-router --json` 顶层 **`skillRuntimeHints.channelSessionDefaults`** 与首响 **`firstResponseContext.channelSessionDefaults`** 的并集语义（以后者为用户可见会话实例）。
...
用户说「退出 / 停止 / 退出福帮手」时，宿主或自动化须执行 `entry.sessionExit` 指向的脚本以写入 `.fbs/onmyagent-resume.json` 与摘要。
...
`codebuddy/channel-manifest.json` 的 `entry` 与 OnMyAgent **对齐同一套脚本路径**，便于双通道宿主复用集成逻辑。
...
- 发布核验清单：[`../../releases/onmyagent-integration-checklist.md`](../../releases/onmyagent-integration-checklist.md)  
- Tier1 与仓库快照：[`tier1-marketplace-faq.md`](./tier1-marketplace-faq.md)  
- 交付链与 Tier2 插件顺序：[`../05-ops/onmyagent-delivery-tier2.md`](../05-ops/onmyagent-delivery-tier2.md)  
