/**
 * host-capability-detect.mjs — OnMyAgent 宿主能力检测器
 * FBS-BookWriter v2.1.2
 *
 * 功能：
 * - 统一探测 OnMyAgent 宿主目录、本地技能市场、已启用插件
 * - 读取 Node.js 与 Git 运行时信息
 * - 输出 .fbs/host-capability.json（缓存 60 分钟）
 *
 * 输出 routingMode：
 *   hybrid       — OnMyAgent 宿主能力 + 脚本双轨可用
 *   script-only  — 仅脚本能力可用
...
import {
  createOnMyAgentHostSnapshot,
  readCachedHostSnapshot,
  writeHostSnapshot,
} from './lib/onmyagent-host-runtime.mjs';
...
export async function detectHostCapability({ bookRoot, cwd, skillRoot, fbsDir, force = false } = {}) {
  ...
  const snapshot = createOnMyAgentHostSnapshot({
