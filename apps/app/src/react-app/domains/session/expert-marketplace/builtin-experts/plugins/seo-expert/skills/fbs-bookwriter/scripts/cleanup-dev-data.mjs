...
const CLEANUP_RULES = {
  ...
  // 最终输出文件
  finalOutput: [
    'fbs-bookwriter-v211-onmyagent.zip',
...
export class DevDataCleaner {
  ...
  async cleanupDuplicateFiles() {
    ...
    if (fs.existsSync(distDir)) {
      ...
      const finalPackage = distFiles.find(f =>
        f === 'fbs-bookwriter-v202-onmyagent.zip'
      );
  ...
  async verifyFinalPackage() {
    const finalPackage = path.join(this.projectRoot, 'dist/fbs-bookwriter-v202-onmyagent.zip');
