#!/usr/bin/env node
/**
 * 运行时建议：esm 阶段须与 session-exit / onmyagent-resume 一致，优先解析 `.fbs/esm-state.md`
 * 正文表格中的 S0–S6（与 `onmyagent-session-snapshot.parseEsmState` 同源），避免仅读 frontmatter
 * 导致与「当前状态」表格不一致（OnMyAgent 第四轮实测：runtime-nudge=S1 vs 表格=S5）。
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { parseEsmState as parseEsmMarkdown, parseFrontmatterCurrentState } from "./onmyagent-session-snapshot.mjs";
