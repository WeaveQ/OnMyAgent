...
v3.1 升级：OnMyAgent 环境优先使用 Tier1 技能（本地市场预装），CodeBuddy 环境自动降至 Tier2。

## 平台检测（启动时执行）

```bash
# 检测 OnMyAgent 本地技能市场是否可用
$marketplaceAvail = Test-Path "~/.onmyagent/skills-marketplace"
# $marketplaceAvail = $true → OnMyAgent 环境，Tier1 可用
# $marketplaceAvail = $false → CodeBuddy 或其他环境，使用 Tier2/web_search
```

## 三层搜索策略（按顺序尝试）

### Layer A：深度结构化调研（Tier1 首选，仅 OnMyAgent）

若书稿评级 L2/L3 或用户说"深度搜索"/"全面调研"，且 OnMyAgent 环境可用：
使用 `deep-research` 技能（`~/.onmyagent/skills-marketplace/skills/deep-research`）：
...
```bash
node ~/.onmyagent/skills-marketplace/skills/wechat-article-search/scripts/search.js "{keyword}"
```
