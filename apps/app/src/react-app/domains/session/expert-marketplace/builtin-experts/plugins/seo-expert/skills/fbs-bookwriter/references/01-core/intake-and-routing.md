...
```
Step 1：读取进度台账
  → 优先读 .fbs/onmyagent-resume.json（v2.1.1 会话快照，含 currentStage/nextSuggested/wordCount）
    如快照存在且 updatedAt 在 24 小时内：直接使用快照信息跳至 Step 4，无需读取全部文件
  → **时间窗对照（避免与记忆摘要混淆）**
    - **恢复卡** `onmyagent-resume.json`：**24 小时内**视为「热恢复」，intake 可优先减少全量扫盘。
...
## OnMyAgent 实测复盘（2026-04-13）：首屏 · 确认 · 退出
...
> 来源：OnMyAgent Windows 会话复盘（`search_file` 对 `.fbs/` 内文件返回 Found 0，但磁盘上文件存在）。

- 在部分宿主上 **`search_file` 默认不进入**以 `.` 开头的目录（`.fbs/`、`.onmyagent/` 等）。因此 **禁止**用 `search_file` 探测 `onmyagent-resume.json`、`esm-state.md`、`session-resume-brief.md` 是否存在于书稿下——会得到 **0 结果**，易误判为「未检测到 FBS 工作区」。
- **正确做法**：
  - **读取台账**：对固定相对路径使用 **`read_file`**（例如 `<bookRoot>/.fbs/onmyagent-resume.json`）；不存在时再走无恢复卡分支。
...
3. **初始化虚拟书房**（WP1→WP2 过渡必做）：运行 `node scripts/init-fbs-multiagent-artifacts.mjs --book-root <本书根>` 构建 `.fbs/`、`deliverables/`、`releases/` 三层底座及基础工件；首次使用或书根变更时执行，已初始化可跳过（详见 [`onmyagent-agent-briefings.md`](./onmyagent-agent-briefings.md) §S3 写作前 team-lead 必做）。
...
| `GLOSSARY.md` | **S0-A完成后，路径确认时**（P0-5修复：明确生成阶段） | 全路径 | 术语锁定文件；触发条件：用户提到专业术语/行业词汇/内部名词时，由 AI 主动创建并在后续写作中维护一致性；`onmyagent-agent-briefings.md` 要求的术语写入目标 |
