...
/**
 * 从 ~/.onmyagent/fbs-book-projects.json 取与当前书稿不同的最近登记项（最多 2 条）
 */
...
/**
 * 技能市场版本戳晚于 host-capability 探测时间时，建议刷新（OnMyAgent 升级后）
 */
export function getCapabilityRefreshRecommendation(fbsDir, onmyagentHome) {
  ...
  const home = onmyagentHome && String(onmyagentHome).trim();
  ...
  if (mtimeMs > detectedMs + 60_000) {
    return {
      recommended: true,
      reason:
        '检测到技能市场版本信息晚于当前宿主能力快照；OnMyAgent 升级后建议刷新能力探测，以匹配插件与市场技能。',
...
export function buildDeliveryPreviewHints(effectiveBookRoot, onmyagentFeatures) {
  ...
  const base = {
    ...
    presentationBridgeSupported: !!onmyagentFeatures?.presentationBridgeSupported,
  };
  if (!onmyagentFeatures?.presentationBridgeSupported) {
    return {
      ...base,
      userFacingActions: [
