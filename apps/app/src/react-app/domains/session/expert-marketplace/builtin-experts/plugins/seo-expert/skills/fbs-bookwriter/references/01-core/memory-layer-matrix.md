...
| **L3 工作区叙事** | `书稿根/.onmyagent/memory/*.md` | `session-exit` 默认追加镜像（可 `--no-onmyagent-mirror` 关闭） | 给人看的日志、排障、审计；**不替代 L1** |
| **L4 用户全局** | `~/.onmyagent/` | 宿主与技能安装 | 技能副本、乐包账本、`settings.json`、二进制登记 |
...
- 退出镜像：见 `session-exit.mjs`（`--no-onmyagent-mirror`）
