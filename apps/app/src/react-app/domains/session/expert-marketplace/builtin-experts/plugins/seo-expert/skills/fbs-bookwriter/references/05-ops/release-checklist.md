...
> **版本**：2.1.2
> **适用**：维护者打 `pack:onmyagent` / `pack:codebuddy` / `pack:release` 前自检。
> **主包约定**：默认主包为 **OnMyAgent**（`npm run pack:release` 仅产出 OnMyAgent 包）。
...
- [ ] 默认发版命令使用 `npm run pack:release`（仅 OnMyAgent 主包）；如需三通道，显式使用 `npm run pack:release:all`。
...
- [ ] OnMyAgent / CodeBuddy 各自 manifest 与插件元数据指向正确技能 ID。
