...
export function runMidtermPerformanceDashboard({ bookRoot, days = 7, weekLabel = null, jsonOut = null, mdOut = null } = {}) {
  ...
  const resume = readJsonIfExists(path.join(fbs, 'onmyagent-resume.json'), {});
  ...
  const checks = [
    ...
    },
    {
      ...
      detail: `onmyagent-resume.modifiedFiles=${Array.isArray(resume?.modifiedFiles) ? resume.modifiedFiles.length : 'missing'}`,
    },
  ];
  ...
  const resumeReadinessRate = score([
    { id: 'resume-exists', ok: fs.existsSync(path.join(fbs, 'onmyagent-resume.json')) },
    ...
    { id: 'resume-updated-in-window', ok: fileUpdatedInWindow(path.join(fbs, 'onmyagent-resume.json'), fromTsMs) },
  ]);
