...
function listHostMemoryDirs(onmyagentDir) {
  const candidates = [
    path.join(onmyagentDir, 'memory'),
    path.join(onmyagentDir, 'memery'),
  ];
  const seen = new Set();
...
function collectHostMemoryFiles(onmyagentDir) {
  const claimed = new Set();
  return listHostMemoryDirs(onmyagentDir).flatMap((dir) => {
...
export class SmartMemoryCore {
   ...
   * 分析OnMyAgent环境变化
   */
  analyzeOnMyAgentChanges(onmyagentDir) {
    const analysis = {
      timestamp: new Date().toISOString(),
      changes: [],
    ...
    // 分析 USER.md 变化
    const userPath = path.join(onmyagentDir, 'USER.md');
    if (fs.existsSync(userPath)) {
      const currentContent = fs.readFileSync(userPath, 'utf8');
      const previousHash = this.memory.organizationLayer.onmyagentChanges.find(c => c.file === 'USER.md')?.hash || '';
    ...
    // 分析 IDENTITY.md 变化
    const identityPath = path.join(onmyagentDir, 'IDENTITY.md');
    if (fs.existsSync(identityPath)) {
      const currentContent = fs.readFileSync(identityPath, 'utf8');
      const previousHash = this.memory.organizationLayer.onmyagentChanges.find(c => c.file === 'IDENTITY.md')?.hash || '';
      const currentHash = crypto.createHash('md5').update(currentContent).digest('hex');
    ...
    const hostMemoryFiles = collectHostMemoryFiles(onmyagentDir);
    const previousFileCount = this.memory.organizationLayer.onmyagentChanges.find(c => c.type === 'host-memory' || c.type === 'memery')?.count || 0;
    const existingDirs = listHostMemoryDirs(onmyagentDir).filter((dir) => fs.existsSync(dir));
  ...
  cleanupOldData(options = {}) {
    ...
    // 清理 OnMyAgent 变化记录
    const maxOnMyAgentChanges = 100;
    if (this.memory.organizationLayer.onmyagentChanges.length > maxOnMyAgentChanges) {
      const removed = this.memory.organizationLayer.onmyagentChanges.splice(maxOnMyAgentChanges);
      cleanupTasks.push({
        type: 'old_onmyagent_changes',
...
选项:
  ...
  --onmyagent-dir       OnMyAgent 目录 (用于 analyze-wb)
...
示例:
  ...
  try {
    ...
    switch (action) {
      ...
      case 'analyze-wb':
        if (!options.onmyagentDir) {
          console.error('错误: 请提供 OnMyAgent 目录 (--onmyagent-dir)');
          process.exit(1);
        }
        const analysisResult = core.analyzeOnMyAgentChanges(options.onmyagentDir);
        console.log('OnMyAgent 变化分析:');
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    } else if (arg === '--onmyagent-dir' && args[i + 1]) {
      options.onmyagentDir = args[++i];
