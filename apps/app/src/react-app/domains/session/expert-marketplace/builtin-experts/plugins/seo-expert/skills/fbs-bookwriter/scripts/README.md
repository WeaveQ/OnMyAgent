...
| `host-capability-detect.mjs` | 探测 OnMyAgent / CodeBuddy 宿主能力与 Tier1/2 可用性 | 启动前、切换宿主后、打包前校验 |
| `onmyagent-session-snapshot.mjs` | 生成 `.fbs/onmyagent-resume.json` 恢复卡 | 阶段完成、退出前、跨会话恢复 |
...
| `onmyagent-user-profile-bridge.mjs` | 吸收宿主画像到当前书稿上下文 | 首响恢复、偏好同步 |
...
| `pack-onmyagent-marketplace.mjs` | 生成 OnMyAgent 审核包 | 提审 OnMyAgent 时 |
| `pack-codebuddy-plugin.mjs` | 生成 CodeBuddy 插件包 | 发布 CodeBuddy 通道时 |
| `pack-release.mjs` | 默认仅生成 OnMyAgent 主包；可用 `--all` 生成三通道发布产物 | 正式发版、重打送审包 |
...
4. 若涉及退出 / 恢复链路，至少联动检查：
   ...
   - `onmyagent-session-snapshot.mjs`
