...
# FBS-BookWriter × OnMyAgent 技能生态整合
...
**关键规则**：
- 调用任何技能前，**必须先检查 Tier1 本地市场**（`~/.onmyagent/skills-marketplace/skills/`）
- 本地有匹配 → `cp -r` 激活，不走网络
...
```bash
# Tier1 检测（本地市场）
ls ~/.onmyagent/skills-marketplace/skills/{skill-name}/ 2>/dev/null \
  && echo "TIER1_AVAILABLE" || echo "TIER1_NOT_FOUND"

# Tier2 检测（已安装插件）
ls ~/.onmyagent/skills/{skill-name}/ 2>/dev/null \
...
**Tier1 激活**（推荐：零网络延迟）：
```bash
cp -r ~/.onmyagent/skills-marketplace/skills/{skill-name} \
      {bookRoot}/.codebuddy/skills/{skill-name}
# 或直接从原路径读取 SKILL.md 调用
```

**Tier2 调用**：直接通过宿主插件系统调用已安装技能（OnMyAgent UI 可见）。
...
此记录用于：
- 防止重复激活（幂等性保证）
- 会话快照（onmyagent-resume.json）中记录当前已激活 Provider 列表
