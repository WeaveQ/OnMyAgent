...
```bash
ls ~/.onmyagent/skills-marketplace/skills/ | grep "{关键词}"
# 或查阅 skill-registry.csv
cat ~/.onmyagent/skills-marketplace/skill-registry.csv | grep "{关键词}"
```

本地有匹配 → `cp -r ~/.onmyagent/skills-marketplace/skills/{name} .codebuddy/skills/`，**零网络延迟激活**。

### Tier2（已安装插件）

检查 `~/.onmyagent/skills/` 目录中的已安装技能。
