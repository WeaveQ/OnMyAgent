...
export class SmartMemoryNatural {
  ...
  parseCommand(command) {
    ...
    if (command.includes('分析') || command.includes('检查') || command.includes('评估') || command.includes('变化')) {
      if (command.includes('onmyagent') || command.includes('工作台') || command.includes('环境')) {
        return { action: 'analyze', type: 'onmyagent', target: null };
      } else {
  ...
  executeAnalyze(parsed) {
    ...
    if (type === 'onmyagent') {
      // 分析 OnMyAgent 变化
      const changes = this.memory.organizationLayer.onmyagentChanges;

      if (changes.length === 0) {
        return {
          success: true,
          message: '没有检测到 OnMyAgent 环境变化。',
...
参数:
  <file>           文件路径（用于学习功能）
  --onmyagent-dir   OnMyAgent 目录（用于分析环境）
