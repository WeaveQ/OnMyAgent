...
> **平台适用范围说明（v3.2）**：
> - 本文件仅描述 **OnMyAgent** 场景下的委托话术；正式发布物已拆分为 OnMyAgent / CodeBuddy 双通道。
> - 宿主真值统一来自 `scripts/host-capability-detect.mjs`：读取 `~/.onmyagent/settings.json` 与 `~/.onmyagent/skills-marketplace/skills/`。
> - 详见 [`brand-platform-convention.md`](./brand-platform-convention.md)。


> **⚠️ 重要：多成员模式选用说明**：以下话术适用于 OnMyAgent 中已支持 Agent Teams / Sub-Agent 的场景。Full Team 本身可直接使用；是否采用并行，取决于任务拆分、写入隔离、心跳与交接设计，而不是宿主“不支持并行”。
>
...
>
> **执行约束全文**：见 [`skill-authoritative-supplement.md §宿主集成指南`](./skill-authoritative-supplement.md)、[OnMyAgent 官方说明](https://www.codebuddy.cn/docs/onmyagent/Overview)
>
> **用途**：给想在 OnMyAgent 中通过自然语言协调多任务时使用的轻量委托话术；**不**执行 API。
>
> **技能生态（v3.2 重要）**：
> - **OnMyAgent 环境**：本地预装技能市场（`~/.onmyagent/skills-marketplace/skills/`）作为 Tier1，已启用插件作为 Tier2；优先本地市场，其次插件，最后远程发现。
> - 运行时平台检测：`Test-Path ~/.onmyagent/skills-marketplace`，并继续读取 `settings.json` 确认可用插件。
>
> **关键 OnMyAgent Tier1 技能（可直接探测）**：
...
>
> **多 Agent 说明（v3.2）**：OnMyAgent 通道支持 Sub-Agent（`.codebuddy/agents/*.md` 声明式定义）和 Agent Teams（`team_create`/`send_message`）。S3/S5 可直接使用 Team API；正文写作默认建议逐章委派或由 team-lead 控编，若用户明确要求且章节边界清晰，也可直接启用 Full Team。关键约束是编排与写入边界，而不是禁止并行。
...
6. **状态 UI**：写前确认 **`[S3-Ch章]`** 对应 md 文件内容为 **非空白内容**，若 OnMyAgent 展示工具调用失败 unknown，以**磁盘为准**，必要时运行 `node scripts/verify-expected-artifacts.mjs --book-root <本书根> --expect-chapter <N> --strict`
```
...
为确保每批章节正确完成，S3 结束后 OnMyAgent 宿主执行核对：
...
> **使用方式**：以下话术仅供应对复杂 OnMyAgent 任务时使用，不要逐条朗读；仅在需要协调分工时使用
