...
> **用途**：约束当前 `OnMyAgent Marketplace` 与 `CodeBuddy Plugin` 双通道中的品牌、平台和宿主能力表述
> **数据来源**：`host-capability-detect.mjs`、`onmyagent/channel-manifest.json`、`codebuddy/channel-manifest.json`
...
- **OnMyAgent Marketplace 包**：`dist/fbs-bookwriter-v211-onmyagent.zip`
...
| 提及宿主平台 | **OnMyAgent** / **CodeBuddy** | 只在明确说明通道差异时出现 |
| 插件包内部元数据 | **`fbs-bookwriter`** | 与 `SKILL.md` frontmatter `name` 对齐 |

### 2.2 禁止示例

```text
❌ 「当前还是单一 OnMyAgent 审核包」
❌ 「一个包同时表示 OnMyAgent / CodeBuddy 的全部能力」
❌ 「minimax-xlsx 是 OnMyAgent Tier1」
...
```text
✅ 「福帮手已就绪，你要写什么？」
✅ 「当前在 OnMyAgent 通道，已检测到本地市场与已启用插件」
...
| 能力 | OnMyAgent Marketplace | CodeBuddy Plugin | 说明 |
|------|-----------------------|------------------|------|
| 通道清单 | `onmyagent/channel-manifest.json` | `codebuddy/channel-manifest.json` | 各自的随包真值 |
...
| Tier1 本地市场 | ✅ `~/.onmyagent/skills-marketplace/` | ❌ | 仅 OnMyAgent 包使用 |
| Tier2 宿主插件 | ✅ `~/.onmyagent/settings.json` | ✅ 宿主插件与工作区 `.codebuddy/` | 都支持，但来源不同 |
...
- **OnMyAgent**：检查 `~/.onmyagent`、`settings.json`、`skills-marketplace/skills/`
...
- [ ] `_plugin_meta.json` 是否声明 `onmyagent-marketplace` 与 `codebuddy-plugin`
- [ ] `.codebuddy-plugin/plugin.json` 是否随包交付且默认代理为 `fbs-team-lead`
- [ ] `onmyagent/channel-manifest.json` 与 `codebuddy/channel-manifest.json` 是否都已生成
