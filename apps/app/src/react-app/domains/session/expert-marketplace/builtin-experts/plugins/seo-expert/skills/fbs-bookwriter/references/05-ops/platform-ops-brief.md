...
> **补充说明**：OnMyAgent / CodeBuddy 的 Team 模式原生支持并行；这里的路径策略、超时与隔离治理，目标是保证并行可控和可恢复，而不是把 Team 降级为串行单智能体。
...
> 与 [`onmyagent-agent-briefings.md`](../01-core/onmyagent-agent-briefings.md) §协同角色 保持一致
