 ...
 * 提取用户画像
 */
export function extractUserProfile(onmyagentDir) {
  const userProfilePath = path.join(onmyagentDir, 'USER.md');
  const identityPath = path.join(onmyagentDir, 'IDENTITY.md');
  const soulPath = path.join(onmyagentDir, 'SOUL.md');
  const hostMemorySources = resolveHostMemorySources(onmyagentDir);
...
function resolveHostMemorySources(onmyagentDir) {
  const candidateDirs = [
 ...
 */
export function getHostProfileSummaryForBrief(onmyagentDir) {
  if (!onmyagentDir || !fs.existsSync(onmyagentDir)) return '';
  try {
    const profile = extractUserProfile(onmyagentDir);
    const lines = [];
    ...
    return ['## 宿主用户画像摘要（OnMyAgent 用户目录）', '', ...lines, ''].join('\n');
  } catch {
    return '';
  }
}
...
用法:
  node onmyagent-user-profile-bridge.mjs <onmyagent-dir> [options]

参数:
  onmyagent-dir  OnMyAgent 配置目录 (通常是 ~/.onmyagent)
...
示例:
  node onmyagent-user-profile-bridge.mjs ~/.onmyagent
  node onmyagent-user-profile-bridge.mjs ~/.onmyagent --intake-only
  node onmyagent-user-profile-bridge.mjs ~/.onmyagent --json
    `);
    process.exit(0);
  }
  
  const onmyagentDir = args[0];
  const options = {
