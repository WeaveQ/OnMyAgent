...
> **v2.1.0 补充**：OnMyAgent 记忆能力审计结论（6个缺陷 D1-D6）+ 宿主融合增强路线图（E1-E14）
...
- **职责**：
  ...
  - 在**宿主可选桥接脚本存在时**，追加 OnMyAgent 摘要 / 环境快照输出
- **结论**：它是**编排器**，不是记忆内核

### 4. 宿主可选桥接层

- **可选脚本**：
  - `onmyagent-memory-digest.mjs`
  - `onmyagent-environment-snapshot.mjs`
- **职责**：
  - 把宿主侧（如 OnMyAgent）的用户档案、环境信息整理成适合本书工程读取的摘要
  - 生成 `.fbs/onmyagent-memory-digest.json`、`.fbs/onmyagent-environment.json`
...
| 宿主可选桥接层 | `onmyagent-memory-digest.mjs` / `onmyagent-environment-snapshot.mjs` | 否 | 依赖宿主扩展，缺失时允许跳过 |
...
| `onmyagent-user-profile-bridge.mjs` | 宿主桥接能力 | 读取宿主用户画像，为 intake 或个性化提供输入 |
...
> `smart-memory-core.mjs` 负责真值与持久化，`smart-memory-natural.mjs` 负责对话式操作，OnMyAgent 摘要 / 环境快照属于可选宿主桥接增强。
...
- **宿主桥接输出** 以 `onmyagent-memory-digest.mjs` / `onmyagent-environment-snapshot.mjs` 为准，但二者允许缺省
...
| 旧/相关工具 | 当前层级 | 推荐理解 |
|-------------|----------|----------|
| `onmyagent-user-profile-bridge.mjs` | 宿主桥接能力 | 提取宿主画像，供 intake / 个性化参考 |
...
- `onmyagent-user-profile-bridge.mjs`
...
2. **禁止把桥接脚本写成核心真值**
   `onmyagent-memory-digest.mjs` 与 `onmyagent-environment-snapshot.mjs` 只能描述为宿主可选桥接
...
- **宿主可选桥接**：`onmyagent-memory-digest.mjs`、`onmyagent-environment-snapshot.mjs`
- **相邻能力模块**：`style-learning.mjs`、`session-state-manager.mjs`、`template-recommender.mjs`、`onmyagent-user-profile-bridge.mjs`、`exa-search-enhancer.mjs`

这份分层既能解释当前代码现实，也能作为后续文档、脚本注册和测试补齐的统一口径。

---

## v2.1.0 补充：OnMyAgent 记忆能力审计与宿主融合路线图

> **审计日期**：2026-04-11
> **审计范围**：OnMyAgent v1.100.0 原生记忆体系 vs FBS v2.0.2 智能记忆系统
...
| # | 缺陷 | 严重度 | 说明 |
|---|------|--------|------|
| D1 | **与宿主 memery 零交互** | P0 | FBS 从未读取 `~/.onmyagent/memery/`，两套用户画像并行存在但不互通 |
...
### OnMyAgent 原生记忆体系（6层，FBS 当前未充分利用）

| 记忆类型 | 存储位置 | LLM 可达 | 跨项目 | FBS 利用情况 |
|----------|----------|---------|--------|------------|
| 用户画像（memery） | `~/.onmyagent/memery/{UID}_memery.md` | ✅ 系统提示注入 | ✅ | ❌ 未读取 |
| 项目工作日志 | `{workspace}/.onmyagent/memory/` | ✅ 系统提示注入 | ❌ | ✅ 部分使用 |
...
| 编号 | 增强项 | 实现状态 |
|------|--------|----------|
| E1 | 读取 memery 为画像种子（`onmyagent-user-profile-bridge.mjs` 已实现，需在 SKILL.md 入口层激活） | ⚠️ 桥接器存在，未激活 |
...
| 编号 | 增强项 |
|------|--------|
| E10 | 里程碑自动同步到 `.onmyagent/memory/` 日志 |
...
| X4 | 不合并 CLAUDE.md 体系 | memory-spring-cleaning 面向 CodeBuddy 的 CLAUDE.md 生态，与 OnMyAgent 无关 |
