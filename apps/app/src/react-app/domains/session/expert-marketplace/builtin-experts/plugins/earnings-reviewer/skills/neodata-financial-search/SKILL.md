...
代理会自动填充 `request_id` 等字段；`channel` 固定为 `neodata`，`sub_channel` 固定为 `onmyagent`，客户端必须显式传入这两个字段。
...
> - 文档示例用 `python3`；若所在环境无 `python3` 命令（部分 Windows），改用 `python` 或 OnMyAgent 内置 Python 执行即可，脚本逻辑一致。
...
| `sub_channel` | 是 | 子渠道信息，固定值 `onmyagent` |
