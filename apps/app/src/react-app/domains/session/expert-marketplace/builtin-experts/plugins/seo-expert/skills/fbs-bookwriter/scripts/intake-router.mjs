/**
 * intake-router.mjs — FBS 主调度入口路由器（OnMyAgent / CodeBuddy 双通道）
 * FBS-BookWriter v2.1.2
 *
 * 设计目标：
 *   - 用统一的宿主快照指导入口动作
 *   - 恢复优先：优先读取 resume.json / chapter-status.md
 *   - 画像增强：检测到 OnMyAgent 画像时，为开场提供更贴合的建议
...
import { adaptIntakeProtocol, extractUserProfile } from './onmyagent-user-profile-bridge.mjs';
...
import { generateSessionSnapshot } from './onmyagent-session-snapshot.mjs';
...
function buildProfileContext(hostCap) {
  const onmyagentHome = hostCap?.onmyagent?.homeDir;
  if (!onmyagentHome || hostCap?.hostType !== 'onmyagent') return { profile: null, intakeProfile: null };

  try {
    const profile = extractUserProfile(onmyagentHome);
...
function readResumeCard(resolvedFbsDir) {
  const resumePath = path.join(resolvedFbsDir, 'onmyagent-resume.json');
...
function collectNestedBookRoots(startDir, maxDepth, maxNodes, maxResults = 12) {
  ...
  while (queue.length && nodes < maxNodes && hits.length < maxResults) {
    ...
    if (fs.existsSync(path.join(fbs, 'chapter-status.md')) || fs.existsSync(path.join(fbs, 'onmyagent-resume.json'))) {
      hits.push(dir);
    }
    if (depth >= maxDepth) continue;
    let entries;
...
function scoreProjectAnchorCandidate(bookRoot) {
  ...
  push(fs.existsSync(path.join(fbsDir, 'onmyagent-resume.json')), 3, '存在恢复卡');
...
function collectProjectHintsFromLocalMemory(resolvedBookRoot) {
  const files = [
    path.join(resolvedBookRoot, '.fbs', 'onmyagent-resume.json'),
...
function collectExternalDriveFbsRoots(resolvedBookRoot, maxResults = 8) {
  ...
  for (const driveRoot of listWindowsDriveRoots()) {
    ...
    for (const d1 of lv1) {
      ...
      if (fs.existsSync(path.join(p1, '.fbs', 'chapter-status.md')) || fs.existsSync(path.join(p1, '.fbs', 'onmyagent-resume.json'))) {
      ...
      for (const d2 of lv2) {
        ...
        if (fs.existsSync(path.join(p2, '.fbs', 'chapter-status.md')) || fs.existsSync(path.join(p2, '.fbs', 'onmyagent-resume.json'))) {
...
function hasMemoryBriefSource(effectiveBookRoot, effectiveFbsDir, memoryProvider) {
  ...
  return [
    ...
    path.join(effectiveFbsDir, 'onmyagent-resume.json'),
...
async function ensureRuntimeArtifacts(resolvedBookRoot, effectiveBookRoot, effectiveFbsDir, memoryProvider) {
  ...
  const resumeJson = path.join(effectiveFbsDir, 'onmyagent-resume.json');
  ...
  if (!fs.existsSync(resumeJson) && (fs.existsSync(chapterStatus) || fs.existsSync(esmState))) {
    try {
      ...
      if (!resumeSnapshotRegenerated) artifactErrors.push('恢复卡补写后仍未生成 onmyagent-resume.json');
    } catch (error) {
...
function formatRoutingModeLabel(mode) {
  ...
  if (m === 'onmyagent_only' || m === 'onmyagent') return 'OnMyAgent';
...
function buildTermValueGlossary() {
  return [
    { term: '.fbs/onmyagent-resume.json', role: '会话恢复卡', userValue: '下次继续时不必重讲背景，直接从上次断点开工。' },
...
/** 终端用户只看这一行（+3 个选项），勿堆 intake 全文 JSON / SKILL 规范（OnMyAgent 实测 P0-1） */
...
function buildFirstResponseContext(env) {
  ...
  /** 对用户转述用人话，勿照搬 Tier/插件代号；2026-04-13 OnMyAgent 实测：首屏忌冗长菜单（P0-1） */
  ...
  const wbHomeForCap = env.hostCap?.onmyagent?.homeDir || path.join(os.homedir(), '.onmyagent');
  const capabilityRefresh = getCapabilityRefreshRecommendation(env.fbsDir, wbHomeForCap);
  const deliveryAndPreview = buildDeliveryPreviewHints(env.effectiveBookRoot, env.hostCap?.onmyagentFeatures);
  ...
  return {
    ...
    onmyagentGenieVersion: cap?.onmyagentGenieVersion || null,
    recommendedOneLiner,
    userFacingOneLiner,
    blockedDecisionHint,
...
function sanitizeHookErrorMessage(message) {
  ...
  // OnMyAgent 实测：Windows / PowerShell 下缺少 Unix 命令（如 head）时向用户转述为人话（P1-3）
...
export function buildRecommendations(intent, env) {
  ...
  if (env.resumeSnapshotRegenerated) {
    info.push('已自动补写 .fbs/onmyagent-resume.json');
  }
  ...
  const wbHome = env.hostCap?.onmyagent?.homeDir || path.join(os.homedir(), '.onmyagent');
  ...
  if (capRef.recommended) {
    ...
    actions.push({
      ...
      cmd: `node scripts/host-capability-detect.mjs --book-root "${eb}" --json --force`,
      reason: '使已安装插件与市场技能与当前 OnMyAgent 探测结果一致',
      required: false,
    });
  }
  ...
  if (dataCompatibility?.status === 'needs_migration') {
    ...
    actions.push({
      ...
      cmd: `node scripts/onmyagent-session-snapshot.mjs --fbs-dir "${env.fbsDir}" --quiet`,
  ...
  if (env.profile?.isProfileComplete) {
    ...
    info.push(`已载入 OnMyAgent 用户画像：${displayName} / ${currentProject}`);
  }
    ...
    case 'resume': {
      if (!env.hasResumeJson && env.hasChapterStatus) {
        blockers.push({
          ...
          message: '缺少标准恢复卡 `.fbs/onmyagent-resume.json`，且自动补写未成功；续写前必须先生成恢复卡快照。',
        });
        actions.push({
          step: actions.length + 1,
          label: '生成恢复卡快照',
          cmd: `node scripts/onmyagent-session-snapshot.mjs --fbs-dir "${env.fbsDir}" --quiet`,
          reason: '缺少 `.fbs/onmyagent-resume.json`，需先生成标准恢复卡再进入续写',
          required: true,
        });
      }

      const resumeFile = env.hasResumeJson
        ? path.join(env.fbsDir, 'onmyagent-resume.json')
  ...
  const hostKpiSignals = {
    ...
    presentationBridgeReady: env.hostCap?.onmyagentFeatures?.presentationBridgeSupported ? 1 : 0,
...
async function main() {
  ...
  if (args.help) {
    console.log(`
intake-router.mjs — FBS 主调度入口路由器 v2.1.2（OnMyAgent / CodeBuddy 双通道）
...
用法：
  ...
  --search 在 ~/.onmyagent/fbs-book-projects.json 索引中按书名/路径关键词检索历史书稿目录（需曾成功执行过 session-exit）
