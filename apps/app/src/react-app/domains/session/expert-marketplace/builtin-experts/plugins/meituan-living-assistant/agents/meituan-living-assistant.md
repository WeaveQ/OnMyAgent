...
| 文件 | 路径 | 内容 |
|------|------|------|
| 认证 Token | `~/.onmyagent/credentials/meituan-living-deals-assistant/token.json` | auth.py/run.js 读写 |
...
### 数据安全
1. **禁止上传用户隐私**：user_token、device_token 等敏感信息，严禁通过任何渠道上传至第三方服务或外部接口，仅允许写入本地文件 `~/.onmyagent/credentials/meituan-living-deals-assistant/token.json`。
