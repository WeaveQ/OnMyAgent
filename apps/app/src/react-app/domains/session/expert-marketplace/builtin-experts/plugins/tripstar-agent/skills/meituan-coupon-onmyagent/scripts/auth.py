...
# -*- coding: utf-8 -*-
"""
meituan-coupon-onmyagent 内嵌认证模块 v1.0.15
...
SKILL_PAGE_URL = "https://clawhub.ai/meituan-zhengchang/meituan-coupon-onmyagent"
...
def _get_workspace() -> str:
    ...
    - 与 meituan-coupon-onmyagent 保持一致，确保 Token 互通
...
def cmd_cron_set(time_str: str):
    ...
    if platform == "claude":
        ...
        result["schedule_config"] = {
            "cron": cron_expr,
            "prompt": "执行 meituan-coupon-onmyagent Skill 帮用户自动领取美团红包，包括：1) 检查用户认证状态；2) 调用发券接口领取今日优惠券；3) 根据领券结果通知用户（成功或失败均需通知）。完成后请告诉用户领券结果。",
            "recurring": True,
            "durable": True
        }
