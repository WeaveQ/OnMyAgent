...
export function planVersionBump(rootDir, nextVersion) {
  ...
  operations.push(
    ...
    {
      filePath: path.join(root, 'onmyagent', 'channel-manifest.json'),

      apply() {
        const manifest = readJson(this.filePath);
        manifest.skill.version = nextVersion;
        if (manifest.distribution?.primaryPackage) {
          manifest.distribution.primaryPackage = `dist/fbs-bookwriter-v${nextCompact}-onmyagent.zip`;
        }
        if (manifest.distribution?.reviewNote) {
          manifest.distribution.reviewNote = `releases/onmyagent-review-v${nextVersion}.md`;
        }
  ...
  operations.push(
    ...
    {
      filePath: path.join(root, 'scripts', 'pack-onmyagent-marketplace.mjs'),
      type: 'text',
      apply() {
        return updateTextFile(this.filePath, (source) => {
          let output = replaceRegex(source, /version: '[^']+'/, `version: '${nextVersion}'`, 'pack-onmyagent version');
          output = replaceRegex(output, /packageName: 'fbs-bookwriter-v\d+-onmyagent'/, `packageName: 'fbs-bookwriter-v${nextCompact}-onmyagent'`, 'pack-onmyagent packageName');
          output = replaceRegex(output, /releases\/onmyagent-review-v[^']+\.md/, `releases/onmyagent-review-v${nextVersion}.md`, 'pack-onmyagent review file');
          return output;
        });
      },
    },
