...
3. 若书房已存在（`.fbs/` 目录）：自动加载 `esm-state.md` 和 `onmyagent-resume.json` 恢复上下文
...
5. 若存在 `.fbs/onmyagent-resume.json` 且 `updatedAt` 在 24 小时内：优先读快照，跳过全量文件扫描
...
启动时读取 `.codebuddy/providers/provider-registry.yml`。源仓库按 **OnMyAgent / CodeBuddy 双通道分轨**：`.codebuddy-plugin/plugin.json` 对应 `codebuddy/channel-manifest.json`，OnMyAgent 审核包对应 `onmyagent/channel-manifest.json`。

**平台检测（第一步，宿主真值）**：

```powershell
# Windows PowerShell
$isOnMyAgent = Test-Path "$env:USERPROFILE\.onmyagent\skills-marketplace"
```
```bash
# Unix/macOS
IS_ONMYAGENT=$([ -d "$HOME/.onmyagent/skills-marketplace" ] && echo "true" || echo "false")
```
- `$isOnMyAgent = true` → **OnMyAgent** 环境，先读本地市场，再读已启用插件
- `$isOnMyAgent = false` → **CodeBuddy / script-only** 路径，跳过 Tier1，优先使用 Tier2 与内置兜底

**调度规则**（严格执行）：
1. 统一先执行 `node scripts/host-capability-detect.mjs --book-root <bookRoot>`
2. **OnMyAgent 环境**：先检查 Tier1（`~/.onmyagent/skills-marketplace/skills/`）
3. **CodeBuddy 环境**：直接检查 Tier2（宿主插件）与仓库内脚本能力
4. Tier1 / Tier2 不可用时 → 尝试 Tier3（远程发现，`find-skills`）或使用 `fallback`
5. 每次使用技能后告知用户所走链路：`OnMyAgent 本地市场 / 宿主插件 / 内置兜底`
...
| 中国主题 / 中文资讯 | `wechat-search`(Tier1) 建议启用（以 OnMyAgent 本地市场探测结果为准）|
...
- `.fbs/onmyagent-resume.json` — 会话快照（由 `onmyagent-session-snapshot.mjs` 生成）
...
## 本地市场技能激活方式（仅 OnMyAgent 环境）

若某 Tier1 技能尚未安装（OnMyAgent 环境下），优先通过 OnMyAgent 本地市场完成安装，再由 `host-capability-detect.mjs` 重新探测可用性。
