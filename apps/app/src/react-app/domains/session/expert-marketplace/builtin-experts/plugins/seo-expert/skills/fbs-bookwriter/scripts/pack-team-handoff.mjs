 ...
 * 非上架技能包；上架请 npm run pack:onmyagent
 */
import fs from 'fs';
import os from 'os';
...
/** 自仓库根目录复制的文件（保留相对路径） */
const EXTRA_FILES = [
  'releases/onmyagent-integration-checklist.md',
  'releases/onmyagent-review-v2.1.1.md',
  'releases/codebuddy-review-v2.1.1.md',
  'releases/README.md',
  'references/05-ops/credits-guide.md',
  'references/05-ops/ux-agent-playbook.md',
  'references/06-plugin/onmyagent-host-integration.md',
...
> **版本**：2.1.1（与 OnMyAgent 过审技能包版本对齐）
> **打包时间**：${packagedAt}
...
| **上架技能包** | \`npm run pack:onmyagent\` → \`dist/fbs-bookwriter-v211-onmyagent.zip\`，用于送审与分发 |
...
- 宿主展示：\`references/06-plugin/onmyagent-host-integration.md\`、\`fbs-runtime-hints.json\`
  ...
  try {
    ...
    for (const rel of EXTRA_FILES) {
      const label =
        ...
        rel.includes('onmyagent-review') ? 'OnMyAgent 通道 review 备忘' :
        ...
        rel.includes('onmyagent-host-integration') ? 'OnMyAgent 宿主集成契约' :
        ...
        rel.includes('channel-manifest.json') ? 'OnMyAgent 通道清单（入口脚本路径）' :
    ...
    console.log('');
    console.log('上架技能包请使用：npm run pack:onmyagent');
  } finally {
