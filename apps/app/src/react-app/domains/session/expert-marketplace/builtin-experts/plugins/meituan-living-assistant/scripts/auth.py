...
def _resolve_auth_file() -> Path:
    """
    跨平台确定存储路径，优先级：
    1. 环境变量 ONMYAGENT_AUTH_FILE（显式指定，最高优先级）
    2. ~/.onmyagent/credentials/meituan-order/token.json
    """
    import os

    env_path = os.environ.get("ONMYAGENT_AUTH_FILE")
    if env_path:
        return Path(env_path)

    return Path.home() / ".onmyagent" / "credentials" / "meituan-living-deals-assistant" / "token.json"
