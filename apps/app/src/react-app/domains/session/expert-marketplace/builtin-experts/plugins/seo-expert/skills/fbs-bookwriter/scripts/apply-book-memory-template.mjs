...
import { getHostProfileSummaryForBrief } from './onmyagent-user-profile-bridge.mjs';
...
选项：
  ...
  --dry-run           只输出预览，不写入文件
  --include-host-profile  在 session-resume-brief 顶部追加 ~/.onmyagent 用户画像短摘要（与退出流程默认行为对齐）
  --help, -h          显示帮助
...
宿主桥接说明（MEM-P2-02）：
  宿主（OnMyAgent / CodeBuddy 等）在新会话启动时，可执行本脚本为 AI 注入记忆上下文。
...
export function applyBookMemoryTemplate({
  ...
  if (includeHostProfileInBrief) {
    const home = onmyagentHome || path.join(os.homedir(), '.onmyagent');
