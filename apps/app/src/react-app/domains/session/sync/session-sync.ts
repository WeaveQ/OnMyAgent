...
type SyncEntry = {
  ...
  pendingDeltas: Map<
    string,
    { messageId: string; reasoning: boolean; text: string }
  >;
...
function eventErrorMessage(properties: unknown) {
  ...
  if (error && typeof error === "object") {
    ...
    if (nestedData && typeof nestedData === "object") {
      const nestedMessage = "message" in nestedData ? nestedData.message : null;
      if (typeof nestedMessage === "string" && nestedMessage.trim()) {
        return nestedMessage.trim();
      }
    }
    const detail = errorRecord.detail ?? errorRecord.reason;
...
function isTrackedSession(entry: SyncEntry, sessionId: string) {
  return (
    (entry.trackedSessionRefs.get(sessionId) ?? 0) > 0 ||
    entry.retainedSessionTimers.has(sessionId)
  );
}
...
function clearTrackedSession(
  ...
  queryClient.removeQueries({
    queryKey: permissionKey(input.workspaceId, sessionId),
    exact: true,
  });
...
function questionWithReceivedAt(
...
export function seedQuestionState(
  ...
  useSessionActivityStore.getState().replaceWaitingRequests(
    ...
    questions.flatMap((question) =>
      question.sessionID === sessionId ? [question.id] : [],
    ),
  );
  const queryClient = getReactQueryClient();
  const now = Date.now();
    ...
    (current = []) => {
      ...
      const seeded = questions.flatMap((question) =>
        question.sessionID === sessionId
          ? [
              questionWithReceivedAt(
                question,
                receivedAtById.get(question.id) ?? now,
              ),
            ]
          : [],
      );
...
function toFileSourceUIPart(part: FilePart): UIMessage["parts"][number] | null {
  ...
  const providerMetadata = {
    opencode: { partId: sourceId, sourcePartId: part.id, source },
  };
  ...
  if (part.type === "tool") {
    if (part.tool === STRUCTURED_OUTPUT_TOOL) {
      ...
      return {
        ...
        providerMetadata: {
          opencode: {
            partId: `structured-output-${part.callID}`,
            toolPartId: part.id,
          },
        },
      };
...
function toUIParts(part: Part): UIMessage["parts"] {
  ...
  if (!mapped) return [];
  if (part.type === "tool" && part.tool === STRUCTURED_OUTPUT_TOOL)
    return [mapped];
  if (
    part.type === "tool" &&
...
function getPartMetadataId(part: UIMessage["parts"][number]) {
  if (part.type === "dynamic-tool") {
    ...
    return "partId" in metadata
      ? ((metadata as { partId?: string }).partId ?? null)
      : null;
  }
  if (
    part.type !== "text" &&
...
function upsertPart(
  ...
  return messages.map((message) => {
    ...
    const index = message.parts.findIndex(
      (part) =>
        ("toolCallId" in part && part.toolCallId === partId) ||
...
function applyEvent(
  ...
  if (event.type === "session.updated") {
    ...
    if (!update) return;
    // NOTE: do NOT gate this on `isTrackedSession`. OpenCode emits
    // `session.updated` whenever a session's metadata (title, time, etc.)
    // changes, including the LLM-generated title that is produced right
    // after the first assistant response. Brand-new sessions race against
    // the React `useEffect` that registers tracking, so gating here
    // silently drops the title update and the sidebar is stuck on
    // "新建任务" / "New session". The downstream listener in
    // `session-route.tsx` (`handleRuntimeSessionUpdated`) already no-ops
    // when the sessionId isn't in the sidebar list.
  ...
  if (event.type === "session.next.compaction.started") {
    ...
    if (sessionId)
      useSessionActivityStore
        .getState()
        .setCompacting(workspaceId, sessionId, true);
    return;
  }
  ...
  if (
    ...
    if (sessionId)
      useSessionActivityStore
        .getState()
        .setCompacting(workspaceId, sessionId, false);
    return;
  }
  ...
  if (event.type === "session.status") {
    ...
    if (tracked)
      queryClient.setQueryData(
        statusKey(workspaceId, props.sessionID),
        props.status,
      );
    for (const listener of entry.sessionStatusListeners)
      listener({ sessionId: props.sessionID, status: props.status });
  ...
  if (event.type === "todo.updated") {
    ...
    queryClient.setQueryData(
      todoKey(workspaceId, props.sessionID),
      props.todos,
    );
    return;
  }
  ...
  if (event.type === "permission.asked") {
    ...
    queryClient.setQueryData<PendingPermission[]>(
      permissionKey(workspaceId, permission.sessionID),
      (current = []) => {
        const existing = current.find((item) => item.id === permission.id);
        const next = withReceivedAt(
          permission,
          existing?.receivedAt ?? receivedAt,
        );
        if (existing) {
          return current
            .map((item) => (item.id === permission.id ? next : item))
  ...
  if (event.type === "question.asked") {
    ...
    const receivedAt = Date.now();
    queryClient.setQueryData<PendingQuestion[]>(
      questionKey(workspaceId, question.sessionID),
      (current = []) => {
        const existing = current.find((item) => item.id === question.id);
        const next = questionWithReceivedAt(
          question,
          existing?.receivedAt ?? receivedAt,
        );
        if (existing) {
          return current
            .map((item) => (item.id === question.id ? next : item))
            .sort(sortQuestions);
        }
  ...
  if (event.type === "question.replied" || event.type === "question.rejected") {
    ...
    if (!isTrackedSession(entry, props.sessionID)) return;
    queryClient.setQueryData<PendingQuestion[]>(
      questionKey(workspaceId, props.sessionID),
      (current = []) =>
  ...
  if (event.type === "message.part.updated") {
    ...
    const seededPart =
      pending && (mapped.type === "text" || mapped.type === "reasoning")
        ? {
            ...mapped,
            text:
              pending.text.length > mapped.text.length
                ? pending.text
                : mapped.text,
            state: "streaming" as const,
          }
    ...
    queryClient.setQueryData<UIMessage[]>(
      ...
      (current = []) => {
        ...
        const withMessage = upsertMessage(current, {
          id: part.messageID,
          role,
          parts: [],
        });
        ...
        for (const attachment of attachments) {
          ...
          if (attachmentId)
            next = upsertPart(next, part.messageID, attachmentId, attachment);
        }
        return next;
      },
    );
  ...
  if (event.type === "message.part.delta") {
    ...
    useSessionActivityStore
      .getState()
      .markAssistantOutput(workspaceId, props.sessionID, props.messageID, {
  ...
  if (event.type === "session.idle") {
    ...
    for (const listener of entry.sessionStatusListeners)
      listener({ sessionId: props.sessionID, status: idleStatus });
    if (input && tracked)
      releaseRetainedSessionSoon(input, entry, props.sessionID);
    return;
  }
...
export function ensureWorkspaceSessionSync(input: SyncOptions) {
  ...
  if (existing) {
    ...
    if (input.onSessionUpdated)
      existing.sessionUpdatedListeners.add(input.onSessionUpdated);
  ...
  syncs.set(key, {
    ...
    sessionUpdatedListeners: new Set(
      input.onSessionUpdated ? [input.onSessionUpdated] : [],
    ),
    sessionStatusListeners: new Set(
      input.onSessionStatus ? [input.onSessionStatus] : [],
    ),
...
function releaseWorkspaceSessionSync(input: SyncOptions) {
  ...
  if (!existing) return;
  if (input.onSessionUpdated)
    existing.sessionUpdatedListeners.delete(input.onSessionUpdated);
  if (input.onSessionStatus)
    existing.sessionStatusListeners.delete(input.onSessionStatus);
  existing.refs -= 1;
...
export function seedSessionState(
  ...
  queryClient.setQueryData(
    statusKey(workspaceId, snapshot.session.id),
    snapshot.status,
  );
  queryClient.setQueryData(
    todoKey(workspaceId, snapshot.session.id),
    snapshot.todos,
  );
}

export function trackWorkspaceSessionSync(
  input: SyncOptions,
