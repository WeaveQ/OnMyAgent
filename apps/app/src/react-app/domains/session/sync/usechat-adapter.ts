...
function eventErrorText(properties: unknown) {
  ...
  if (error && typeof error === "object") {
    ...
    if (data && typeof data === "object") {
      const nestedMessage = "message" in data ? data.message : null;
...
function handleEventChunk(
  ...
  if (event.type === "session.error") {
    ...
    const errorText = eventErrorText(event.properties);
