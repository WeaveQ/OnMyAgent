...
const ENGINE_CUSTOM_BIN_KEY = "onmyagent.engineCustomBinPath";
...
export function useDebugViewModel(options: UseDebugViewModelOptions) {
  ...
  const runtimeSummary = useMemo(
    () => ({
      ...
      onmyagentServerVersionLabel: onmyagentServerSnapshot.onmyagentServerDiagnostics?.version ?? "—",
    }),
    [
      appBuild?.gitSha,
      appBuild?.version,
      engineInfoState?.baseUrl,
      onmyagentServerSnapshot.onmyagentServerDiagnostics?.version,
    ],
  );
  ...
  const runtimeDebugReport = useMemo(() => {
    return {
      ...
      onmyagentServer: {
        hostInfo: onmyagentServerSnapshot.onmyagentServerHostInfo,
        diagnostics: onmyagentServerSnapshot.onmyagentServerDiagnostics,
        capabilities: onmyagentServerSnapshot.onmyagentServerCapabilities,
        settings: onmyagentServerSnapshot.onmyagentServerSettings,
        status: onmyagentServerSnapshot.onmyagentServerStatus,
        url: onmyagentServerSnapshot.onmyagentServerUrl,
      },
      runtimeWorkspaceId,
      selectedWorkspaceRoot,
    };
    ...
    appBuild,
    engineInfoState,
    onmyagentServerSnapshot.onmyagentServerCapabilities,
    onmyagentServerSnapshot.onmyagentServerDiagnostics,
    onmyagentServerSnapshot.onmyagentServerHostInfo,
    onmyagentServerSnapshot.onmyagentServerSettings,
    onmyagentServerSnapshot.onmyagentServerStatus,
  ...
  const onExportRuntimeDebugReport = useCallback(async () => {
    try {
      downloadTextAsFile(
        `onmyagent-runtime-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
        runtimeDebugReportJson,
        "application/json",
      );
  ...
  const onExportDeveloperLog = useCallback(async () => {
    try {
      downloadTextAsFile(
        `onmyagent-developer-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        developerLog.join("\n"),
        "text/plain",
      );
  ...
  const onResolveElectronAlphaArtifact = useCallback(async () => {
    setElectronMigrationStatus("Legacy migration controls were removed after Electron became the desktop runtime.");
  }, []);
  ...
  const onRevealElectronMigrationBackup = useCallback(async () => {
    ...
    try {
      ...
      if (!appBundlePath) {
        setElectronMigrationStatus("Could not resolve the current OnMyAgent.app bundle path.");
        return;
      }
      await revealDesktopItemInDir(`${appBundlePath}.migrate-bak`);
      setElectronMigrationStatus("Requested Finder reveal for OnMyAgent.app.migrate-bak. The backup exists after an install handoff completes.");
    } catch (error) {
      setElectronMigrationStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, []);
  ...
  const onPrepareElectronMigrationSnapshot = useCallback(async () => {
    setElectronMigrationStatus("Legacy migration snapshots are no longer available because the previous desktop shell has been removed.");
  }, []);

  const onInstallElectronPreviewFromLegacy = useCallback(async () => {
    setElectronMigrationStatus("Legacy install handoff is no longer available because Electron is now the desktop runtime.");
  }, []);
  ...
  const onRunSandboxDebugProbe = useCallback(async () => {
    ...
    try {
      ...
      if (!result) {
        setSandboxProbeStatus(t("settings.sandbox_error"));
        pushDeveloperLog("sandbox probe ready=false");
        return;
      }
  ...
  const bootFullEngineStack = useCallback(async () => {
    ...
    // Collect ALL local workspace paths so onmyagent-server is started with
    ...
    const info = await engineStartCmd(workspacePath, {
      ...
      onmyagentRemoteAccess:
        optionsRef.current.onmyagentServerSnapshot.onmyagentServerSettings
          .remoteAccessEnabled === true,
    });

    // engine_start restarts onmyagent-server on a NEW port and lets that server
    // manage OpenCode. Re-read host info and persist the fresh URL/token.
    try {
      const hostInfo = (await onmyagentServerInfoCmd()) as {
        baseUrl?: string;
        ownerToken?: string;
        clientToken?: string;
        hostToken?: string;
  ...
  const onExportOpencodeLogs = useCallback(async () => {
    ...
    try {
      downloadTextAsFile(
        `onmyagent-opencode-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        text,
        "text/plain",
      );
  ...
  const onExportOpenworkLogs = useCallback(async () => {
    ...
    try {
      downloadTextAsFile(
        `onmyagent-server-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        text,
        "text/plain",
      );
  ...
  const onOpenResetModal = useCallback(
    (mode: ResetModalMode) => {
      ...
      const message =
        mode === "all"
          ? "Reset ALL OnMyAgent app data? Open sessions and workspaces will be removed."
      ...
      void resetOpenworkState(mode)
        .then(async () => {
          ...
          pushDeveloperLog(`reset_onmyagent_state mode=${mode}`);
        })
        .catch((error) => {
  ...
  const onNukeOpenworkAndOpencodeConfig = useCallback(async () => {
    ...
    const confirmed =
      typeof window === "undefined"
        ? true
        : window.confirm(
            "Delete ALL local OnMyAgent + OpenCode config and quit? This cannot be undone.",
          );
    if (!confirmed) return;
  ...
  const debugProps: DebugViewProps = useMemo(
    () => ({
      ...
      startupLabel:
        onmyagentServerSnapshot.onmyagentServerStatus === "connected"
          ? t("settings.onmyagent_server_label")
      ...
      opencodeRestarting,
      onmyagentServerRestarting,
      opencodeServiceStatus,
      onmyagentServiceStatus,
      opencodeLogStatus,
      onmyagentLogStatus,
      onCopyOpencodeLogs,
      ...
      onmyagentAuditEntries: onmyagentServerSnapshot.onmyagentAuditEntries,
      onmyagentAuditStatus: auditStatusPill(onmyagentServerSnapshot.onmyagentAuditStatus),
      onmyagentAuditError: onmyagentServerSnapshot.onmyagentAuditError,
    ...
    [
      ...
      startupStatus,
      workspaceDebugEventsStatus,
      onmyagentServerSnapshot.onmyagentAuditEntries,
      onmyagentServerSnapshot.onmyagentAuditError,
      onmyagentServerSnapshot.onmyagentAuditStatus,
      onmyagentServerSnapshot.onmyagentServerCapabilities,
      onmyagentServerSnapshot.onmyagentServerDiagnostics,
      onmyagentServerSnapshot.onmyagentServerStatus,
      resetModalBusy,
