...
export function useMessagingViewProps(
  ...
  const serverReady =
    ...
    Boolean(options.onmyagentServerClient);
  ...
  const loadAgentFile = useCallback(async () => {
    ...
    const client = options.onmyagentServerClient;
    if (!client) return;
  ...
  }, [agentLoading, options.onmyagentServerClient, resetAgentState, serverReady, workspaceId]);
  ...
  }, [agentSaving, options.onmyagentServerClient, serverReady, workspaceId]);
  ...
  }, [agentBaseUpdatedAt, agentDraft, agentSaving, options.onmyagentServerClient, serverReady, workspaceId]);
  ...
  const refreshAll = useCallback(async (nextOptions?: { force?: boolean }) => {
    ...
    try {
      ...
      if (isOpenCodeRouterSnapshot(healthRes.json)) {
        ...
        setHealth(null);
        if (!healthRes.ok) {
          const message = t("identities.health_unavailable_status", undefined, {
  ...
  const repairAndReconnect = useCallback(async () => {
    if (options.onmyagentReconnectBusy) return;
  ...
  }, [messagingSaving, options.onmyagentServerClient, refreshAll, serverReady, workspaceId]);
  ...
  const upsertTelegram = useCallback(async (access: "public" | "private") => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const username = getTelegramUsernameFromWriteResult(result);
        if (username) {
          const normalized = username.trim().replace(/^@+/, "");
  ...
  }, [options.onmyagentServerClient, refreshAll, serverReady, telegramSaving, workspaceId]);
  ...
  }, [options.onmyagentServerClient, refreshAll, serverReady, slackSaving, workspaceId]);
  ...
  return {
    ...
    refreshing,
    onmyagentReconnectBusy: options.onmyagentReconnectBusy,
    reconnectStatus,
    reconnectError,
    health,
    healthError,
