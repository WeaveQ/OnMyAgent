...
export function AdvancedFeatureFlagsSection(props: AdvancedFeatureFlagsSectionProps) {
  return (
    <LayoutSection>
      <LayoutSectionHeader>
        <LayoutSectionTitle>{t("settings.feature_flags")}</LayoutSectionTitle>
        <LayoutSectionDescription>{t("settings.feature_flags_desc")}</LayoutSectionDescription>
      </LayoutSectionHeader>

      <LayoutSectionItem>
        <LayoutSectionItemHeader>
          <LayoutSectionItemTitle>{t("settings.create_sandbox_microsandbox")}</LayoutSectionItemTitle>
          <LayoutSectionItemDescription>
          ...
          <LayoutSectionItemHeaderActions>
            <Switch
              aria-label={t("settings.create_sandbox_microsandbox")}
          ...
          {props.deepLinkOpen ? (
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="text-xs text-dls-secondary">{t("settings.deeplink_hint")}</div>
              </div>
            </div>
          ) : null}
  ...
  return (
    <LayoutSection>
      ...
      <LayoutSectionItem className="gap-3">
        <div className="break-all font-mono text-xs text-dls-secondary">{props.baseUrl}</div>
        <div className="flex flex-wrap gap-2 pt-2">
          <Button
            ...
            disabled={props.busy || props.onmyagentReconnectBusy || !props.onmyagentServerUrl.trim()}
          >
            <RefreshCcw size={14} className={props.onmyagentReconnectBusy ? "animate-spin" : ""} />
            {props.onmyagentReconnectBusy ? t("settings.reconnecting") : t("settings.reconnect_server")}
          </Button>
          ...
          {props.isLocalEngineRunning ? (
            ...
            >
              ...
              {props.restartBusy ? t("settings.restarting") : t("settings.restart_onmyagent_server")}
            </Button>
          ) : null}
          ...
          {!props.isLocalEngineRunning && props.onmyagentServerStatus === "connected" ? (
            <Button
              type="button"
              variant="outline"
              size="sm"
