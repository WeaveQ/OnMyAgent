      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-wrap items-center gap-2">
          {props.mcpConnectedAppsCount > 0 ? (
            <StatusBadge tone="accent" className="gap-2" size="default">
              <StatusDot size="md" tone="active" />
              <span>{t("extensions.app_count", { count: props.mcpConnectedAppsCount })}</span>
            </StatusBadge>
          ) : null}
        </div>
          ...
          {pluginCount > 0 ? (
            <details className="group">
              <summary className="flex cursor-pointer items-center gap-2 rounded-lg px-1 py-2 text-sm font-medium text-dls-secondary transition-colors hover:text-dls-text">
                ...
                <CountBadge size="dot">{pluginCount}</CountBadge>
              </summary>
              <div className="mt-3">
                <PluginsView
