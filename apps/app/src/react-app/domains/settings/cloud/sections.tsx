...
interface CloudWorkerListItemProps {
  ...
  worker: CloudWorker;
  onOnMyAgenter: (workerId: string, workerName: string) => void | Promise<void>;
}

function CloudWorkerListItem({ openingWorkerId, worker, onOnMyAgenter }: CloudWorkerListItemProps) {
  ...
  return (
    <SettingsListItem>
      ...
      <Button
        variant="outline"
        size="sm"
        onClick={() => void onOnMyAgenter(worker.workerId, worker.workerName)}
...
export function CloudSkillsSection({
  ...
  const skillGroups = [
    { value: "available", label: t("den.available"), rows: visibleRows.filter((row) => row.status === "available") },
      ...
      {rows.length > 0 ? (
        <>
          ...
          {visibleRows.length > 0 ? (
              ...
              {skillGroups.map((group) => (
                <AccordionItem key={group.value} value={group.value}>
                  <AccordionTrigger className="items-center hover:no-underline group gap-x-3">
                    ...
                    <CountBadge size="compact">{group.rows.length}</CountBadge>
                  </AccordionTrigger>
            ...
            <SettingsListEmptyState>{t("settings.no_skills_match_search")}</SettingsListEmptyState>
          )}
        </>
      ) : null}
    </SettingsSection>
        ...
        >
          ...
          <TabsContent value={selectedMarketplace?.marketplace.id}>
            ...
            {selectedRows.length > 0 && visibleRows.length === 0 ? (
              <SettingsListEmptyState>{t("settings.no_plugins_match_search")}</SettingsListEmptyState>
            ) : null}

            {selectedMarketplace && selectedRows.length === 0 ? (
              <SettingsListEmptyState>{t("settings.marketplace_plugins_empty")}</SettingsListEmptyState>
            ) : null}
          </TabsContent>
        </Tabs>
      ) : null}
      ...
      {workers.length > 0 ? (
        <>
          ...
          {visibleWorkers.length > 0 ? (
            ...
            <SettingsListEmptyState>{t("settings.no_workers_match_search")}</SettingsListEmptyState>
          )}
        </>
      ) : null}
    </SettingsSection>
      ...
      {rows.length > 0 ? (
        <>
          ...
          {visibleRows.length > 0 ? (
            ...
            <SettingsListEmptyState>{t("settings.no_skill_hubs_match_search")}</SettingsListEmptyState>
          )}
        </>
      ) : null}
    </SettingsSection>
      ...
      {rows.length > 0 ? (
        <>
          ...
          {visibleRows.length > 0 ? (
            ...
            <SettingsListEmptyState>{t("settings.no_providers_match_search")}</SettingsListEmptyState>
          )}
        </>
      ) : null}
    </SettingsSection>
