...
declare global {
  interface WindowEventMap {
    "onmyagent-den-session-updated": CustomEvent<DenSessionUpdatedDetail>;
  }
}
...
function parseManualAuthInput(value: string) {
  ...
  try {
    ...
    if (
      (protocol === "onmyagent:" || protocol === "onmyagent-dev:") &&
...
export function useDenSession({
  ...
  const clearSignedInState = React.useCallback(
    (
      ...
      try {
        const raw = window.localStorage.getItem("onmyagent.acknowledgedProviders");
        if (raw) {
