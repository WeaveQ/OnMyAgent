...
import { parseDenAuthDeepLink } from "../../../app/lib/onmyagent-links";
...
export function DenAuthProvider({ children }: DenAuthProviderProps) {
    ...
    try {
      ...
      setError(
        nextError instanceof Error
          ? nextError.message
          : `Failed to restore ${APP_NAME} Cloud session.`,
      );
      setStatus("signed_out");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const handleUrls = (urls: readonly string[]) => {
      for (const rawUrl of urls) {
        ...
        void createDenClient({ baseUrl: parsed.denBaseUrl })
          ...
          .then((result) => {
            if (!result.token) {
              throw new Error(`Failed to sign in to ${APP_NAME} Cloud.`);
            }
            ...
            dispatchDenSessionUpdated({
              ...
              message:
                error instanceof Error
                  ? error.message
                  : `Failed to sign in to ${APP_NAME} Cloud.`,
            });
          });
      }
    };
    ...
    const handleDeepLink = (event: Event) => {
      handleUrls(
        ((event as CustomEvent<DeepLinkBridgeDetail>).detail?.urls ??
          []) as string[],
      );
    };
    ...
    return () =>
      window.removeEventListener(deepLinkBridgeEvent, handleDeepLink);
  }, []);
