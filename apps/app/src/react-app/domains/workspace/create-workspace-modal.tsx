...
export function CreateWorkspaceModal(props: CreateWorkspaceModalProps) {
  ...
  const setScreen = (value: SetStateAction<CreateWorkspaceScreen>) =>
    setLocal("screen", value);
  const setSelectedFolder = (value: SetStateAction<string | null>) =>
    setLocal("selectedFolder", value);
  const setPickingFolder = (value: SetStateAction<boolean>) =>
    setLocal("pickingFolder", value);
  const setShowProgressDetails = (value: SetStateAction<boolean>) =>
    setLocal("showProgressDetails", value);
  const setNow = (value: SetStateAction<number>) => setLocal("now", value);
  const setCloudSettings = (
    value: SetStateAction<ReturnType<typeof readDenSettings>>,
  ) => setLocal("cloudSettings", value);
  const setRemoteUrl = (value: SetStateAction<string>) =>
    setLocal("remoteUrl", value);
  const setRemoteToken = (value: SetStateAction<string>) =>
    setLocal("remoteToken", value);
  const setRemoteDisplayName = (value: SetStateAction<string>) =>
    setLocal("remoteDisplayName", value);
  const setRemoteTokenVisible = (value: SetStateAction<boolean>) =>
    setLocal("remoteTokenVisible", value);
  const setOrgs = (value: SetStateAction<DenOrgSummary[]>) =>
    setLocal("orgs", value);
  const setActiveOrgId = (value: SetStateAction<string>) =>
    setLocal("activeOrgId", value);
  const setOrgsBusy = (value: SetStateAction<boolean>) =>
    setLocal("orgsBusy", value);
  const setOrgsError = (value: SetStateAction<string | null>) =>
    setLocal("orgsError", value);
  const setWorkers = (value: SetStateAction<DenWorkerSummary[]>) =>
    setLocal("workers", value);
  const setWorkersBusy = (value: SetStateAction<boolean>) =>
    setLocal("workersBusy", value);
  const setWorkersError = (value: SetStateAction<string | null>) =>
    setLocal("workersError", value);
  const setOpeningWorkerId = (value: SetStateAction<string | null>) =>
    setLocal("openingWorkerId", value);
  const setWorkerSearch = (value: SetStateAction<string>) =>
    setLocal("workerSearch", value);
  ...
  const modalWidthClass =
    screen === "shared" ? "max-w-2xl sm:max-w-2xl" : "max-w-xl sm:max-w-xl";
  ...
  }, [applyActiveOrg, cloudSettings.activeOrgId, denClient, isSignedIn]);
  ...
  const handleRemoteSubmit = async () => {
    ...
    await Promise.resolve(
      props.onConfirmRemote({
        onmyagentHostUrl: remoteUrl.trim(),
        onmyagentToken: remoteToken.trim() || null,
        directory: null,
  ...
  const handleOnMyAgenter = async (worker: DenWorkerSummary) => {
    if (!props.onConfirmRemote) return;
    const orgId = activeOrgId.trim();
    if (!orgId) {
    ...
    try {
      ...
      const ok = await Promise.resolve(
        props.onConfirmRemote({
          ...
          onmyagentClientToken: tokens.clientToken?.trim() || null,
          onmyagentHostToken: tokens.hostToken?.trim() || null,
          directory: null,
      ...
      >
        ...
        {screen === "remote" ? (
          <>
            ...
            <DialogFooter className="flex-col gap-3">
              {remoteError ? <NoticeBox size="comfortable" tone="error">{remoteError}</NoticeBox> : null}
              <div className="flex justify-end gap-3">
                <DialogClose
                  disabled={remoteSubmitting}
                  render={
                    <Button variant="outline" disabled={remoteSubmitting} />
                  }
                >
                  {t("common.cancel")}
                </DialogClose>
        ...
        {screen === "shared" ? (
          <CreateWorkspaceSharedPanel
            ...
            onOnMyAgenter={(worker) => void handleOnMyAgenter(worker)}
