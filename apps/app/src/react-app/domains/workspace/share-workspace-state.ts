...
export function useShareWorkspaceState(options: UseShareWorkspaceStateOptions) {
  ...
  useEffect(() => {
    const workspace = shareWorkspace;
    const baseUrl = options.onmyagentServerHostInfo?.baseUrl?.trim() ?? "";
    const token =
      options.onmyagentServerHostInfo?.ownerToken?.trim() ||
      options.onmyagentServerHostInfo?.clientToken?.trim() ||
      "";
  ...
  }, [options.onmyagentServerHostInfo, shareWorkspace]);
  ...
  const shareFields = useMemo(() => {
    ...
    if (workspace.workspaceType !== "remote") {
      if (options.onmyagentServerHostInfo?.remoteAccessEnabled !== true) {
        return [];
      }
      const hostUrl =
        options.onmyagentServerHostInfo?.connectUrl?.trim() ||
        options.onmyagentServerHostInfo?.lanUrl?.trim() ||
      ...
      const ownerToken =
        collaboratorToken || options.onmyagentServerHostInfo?.ownerToken?.trim() || "";
      return [
        {
    ...
    if (workspace.remoteType === "onmyagent") {
      ...
      const token =
        ...
        options.onmyagentServerSettings.token?.trim() ||
        "";
      return [
        {
          label: t("session.share_worker_url"),
  ...
  const shareServiceDisabledReason = useMemo(() => {
    ...
    if (workspace.workspaceType === "remote" && workspace.remoteType !== "onmyagent") {
      return t("session.share_onmyagent_workers_only");
    }
    if (workspace.workspaceType !== "remote") {
      const baseUrl = options.onmyagentServerHostInfo?.baseUrl?.trim() ?? "";
      const token =
    ...
    return null;
  }, [options.onmyagentServerHostInfo, options.onmyagentServerSettings, shareWorkspace]);
