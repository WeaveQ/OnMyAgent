...
function remoteSupportMessage(message: string) {
  return `${message} Upgrade the OnMyAgent host and try again. If this continues, contact team@onmyagentlabs.com.`;
}
...
function displayWorkspaceName(workspace: unknown) {
  ...
  return (
    ...
    trim(value.onmyagentWorkspaceName) ||
    trim(value.name) ||
    trim(value.id)
  );
}
...
export function resolveRemoteWorkspaceConnectionTarget(workspace: WorkspaceInfo): TargetResult {
  if (workspace.workspaceType !== "remote") {
    return {
      ...
      state: {
        status: "error",
        message: t("workspace.remote_worker_test_only"),
        checkedAt: Date.now(),
      },
    };
  }

  if (workspace.remoteType && workspace.remoteType !== "onmyagent") {
    return {
      ok: false,
      state: {
        status: "error",
        message: t("workspace.remote_diagnostics_onmyagent_only"),
        checkedAt: Date.now(),
      },
    };
  }

  const rawHostUrl = trim(workspace.onmyagentHostUrl) || trim(workspace.baseUrl);
  if (!rawHostUrl) {
    return {
      ok: false,
      state: {
...
export async function testRemoteWorkspaceConnection(
  ...
  if (!target.token) {
    return fail(
      remoteSupportMessage(`Token is missing for ${target.endpointLabel}. Edit connection and paste a valid OnMyAgent token.`),
      checkedAt,
    );
  }
...
export async function diagnoseRemoteWorkspaceTaskLoadFailure(
  ...
  const checkedAt = options.now?.() ?? Date.now();
  const fallback = redactRemoteDiagnosticText(trim(taskLoadError) || t("app.error_remote_worker_connection_failed"));
