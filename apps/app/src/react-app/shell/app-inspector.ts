 ...
 *
 * Consumers read via `window.__onmyagent` from a devtools console or from
...
declare global {
  ...
  var __onmyagent: InspectorAPI | undefined;
}
...
function installIfNeeded() {
  ...
  Object.defineProperty(window, "__onmyagent", {
    value: api,
    configurable: true,
    writable: false,
  });
...
export function recordInspectorEvent(name: string, data?: unknown) {
  installIfNeeded();
  window.__onmyagent?.record(name, data);
}
