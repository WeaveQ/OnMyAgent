 ...
 * localStorage.onmyagent.debug.profilerOverlay = "1" / "0".
 *
 * In prod builds the wrapper is a pass-through (no Profiler overhead) and
 * the overlay renders null.
 *
 * Findings also land on window.__onmyagent.slice("profiler") so external
 * tools can read them.
 */
...
const devProfilerOverlayClass = {
  shell: "pointer-events-auto fixed bottom-3 right-3 z-[1100] w-[280px] overflow-hidden rounded-lg border border-dls-border bg-dls-canvas/95 text-xs text-dls-text backdrop-blur-sm",
  header: "flex items-center justify-between border-b border-dls-border px-2.5 py-1.5",
  titleRow: "flex items-center gap-2",
  monoTitle: "font-mono text-xs text-dls-secondary",
...
//
// Explicit opt-ins:
//   - VITE_ONMYAGENT_PROFILER=1 at `pnpm dev`
//   - window.localStorage.setItem("onmyagent.debug.profiler", "1")
...
const PROFILER_ENABLED = (() => {
  ...
  try {
    if (window.localStorage.getItem("onmyagent.debug.profiler") === "1") return true;
  } catch {
    // ignore
  }
  return false;
...
// window.__onmyagent.slice("profiler") — even for operators who aren't
...
function readOverlayStoredPreference(): boolean | null {
  ...
  try {
    const raw = window.localStorage.getItem("onmyagent.debug.profilerOverlay");
...
function writeOverlayStoredPreference(value: boolean) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem("onmyagent.debug.profilerOverlay", value ? "1" : "0");
  } catch {
    // ignore
  }
}
...
function DevProfilerOverlayVisible({ onHide }: { onHide: () => void }) {
  ...
  return (
    <div
      className={devProfilerOverlayClass.shell}
      role="status"
      aria-live="polite"
    >
      <div className={devProfilerOverlayClass.header}>
        <div className={devProfilerOverlayClass.titleRow}>
          <span className={devProfilerOverlayClass.monoTitle}>
            react profiler
          </span>
          <span className={devProfilerOverlayClass.subtitle}>
            {snapshot.totalCommits} commits · {snapshot.totalActualMs}ms
          </span>
        </div>
        <div className={devProfilerOverlayClass.actions}>
          <button
            type="button"
            className={devProfilerOverlayClass.button}
            onClick={() => {
              state.zonesById.clear();
              state.recent.length = 0;
              emit();
            }}
            title={t("debug.reset_counters")}
          >
            reset
          </button>
          <button
      ...
      {collapsed ? null : (
        <div className={devProfilerOverlayClass.body}>
          {topZones.length === 0 ? (
            <div className={devProfilerOverlayClass.empty}>
              No profiler data yet. Interact with the app.
            </div>
          ) : (
            <table className={devProfilerOverlayClass.table}>
              <thead>
                <tr className={devProfilerOverlayClass.tableHeader}>
                  <th className={devProfilerOverlayClass.thLeft}>zone</th>
                  <th className={devProfilerOverlayClass.thRight}>#</th>
                  <th className={devProfilerOverlayClass.thRight}>last</th>
                  <th className={devProfilerOverlayClass.thRight}>total</th>
                </tr>
              </thead>
              <tbody>
                {topZones.map((zone) => {
