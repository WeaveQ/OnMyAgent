...
function readStoredList(): string[] {
  ...
  try {
    const raw = window.localStorage.getItem("onmyagent.server.list");
...
function readStoredActive(): string {
  if (typeof window === "undefined") return "";
  try {
    const stored = window.localStorage.getItem("onmyagent.server.active");
...
export function ServerProvider({ children, defaultUrl }: ServerProviderProps) {
  ...
  useEffect(() => {
    ...
    // Hosted web deployments served by OnMyAgent must reuse the OpenCode proxy
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("onmyagent.server.list", JSON.stringify(list));
      window.localStorage.setItem("onmyagent.server.active", active);
    } catch {
      // ignore
    }
  }, [active, list]);
  ...
  useEffect(() => {
    ...
    if (isDesktopRuntime() && !active.includes("/opencode")) {
      // Desktop React routes now talk to OnMyAgent server workspace-mounted
