...
function hasBuiltInBrowserPrompt(haystack) {
  return (
    haystack.includes("built-in onmyagent browser") ||
    haystack.includes("onmyagent browser") ||
...
try {
  tmpdir = await mkdtemp(path.join(os.tmpdir(), "onmyagent-browser-entry-"));
      ...
      if (req.method === "POST" && url.pathname.endsWith("/chat/completions")) {
        ...
        if (triesBuiltInBrowser) {
          ...
          writeSse(
            res,
            createTextStream(
              "Using the built-in OnMyAgent Browser to open example.com and read the page title.",
            ),
          );
        } else {
  ...
  await step("session.create", async () => {
    const session = await client.session.create({ title: "OnMyAgent browser-entry test" });
  ...
  await step("assert.built-in-browser-quickstart", async () => {
    assert.equal(sawBuiltInBrowserPrompt, true, "Expected browser quickstart prompt to use the built-in OnMyAgent Browser");
