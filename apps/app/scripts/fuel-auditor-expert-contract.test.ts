...
describe("fuel-auditor expert contract", () => {
  test("ships a deterministic audit protocol and confirmed automation workflow", () => {
    const agent = readExpertFile("agents/fleet-management-specialist.md");
  ...
  test("preview and export identify same-lane outliers and write demo artifacts", () => {
    ...
    const capabilityDir = join(outputDir, "油费稽查");
    ...
    try {
      const preview = spawnSync(
        "python3",
        [script, "--input", fixture, "--output-dir", capabilityDir, "--mode", "preview"],
        { encoding: "utf8" },
      );
      ...
      const board = readFileSync(join(capabilityDir, ".process/fuel-preview.html"), "utf8");
      ...
      const exported = spawnSync(
        "python3",
        [script, "--input", fixture, "--output-dir", capabilityDir, "--mode", "export"],
        { encoding: "utf8" },
      );
      expect(exported.status, exported.stderr).toBe(0);
      const exportBody = JSON.parse(exported.stdout) as { files: string[] };
      expect(exportBody.files.some((file) => file.includes("油费稽查/油费稽查报告_"))).toBe(true);
      expect(exportBody.files.some((file) => file.includes("单车油耗汇总_"))).toBe(true);
      expect(exportBody.files.some((file) => file.includes("异常明细_"))).toBe(true);
      const proposal = readFileSync(
        join(capabilityDir, "automations/proposals/fuel-weekly-scan.json"),
        "utf8",
      );
