...
describe("pod-reconciler expert contract", () => {
  ...
  test("exports POD gaps, fee totals, controlled variances, and drafts", () => {
    ...
    const capabilityDir = join(outputDir, "回单对账");
    try {
      ...
      expect(body.files.some((file) => file.includes("回单对账/对账单_2026-07.xlsx"))).toBe(true);
      expect(body.files.some((file) => file.includes("回单对账/对账单_2026-07.pdf"))).toBe(true);
    } finally {
