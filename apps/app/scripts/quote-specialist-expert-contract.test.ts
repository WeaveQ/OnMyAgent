...
describe("quote-specialist expert contract", () => {
  ...
  test("preview/export calculate three options and clamp the floor", () => {
    ...
    const capabilityDir = join(outputDir, "报价");
    try {
      ...
      expect(body.files.some((file) => file.includes("报价/报价方案_Q-20260723-001.xlsx"))).toBe(true);
      expect(body.files.some((file) => file.includes("报价/报价方案对比_Q-20260723-001.docx"))).toBe(true);
    } finally {
