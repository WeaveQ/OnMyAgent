...
try {
  ...
  await step("session.create", async () => {
    const session = await client.session.create({ title: "OnMyAgent e2e" });
  ...
  await step("session.prompt noReply", async () => {
    await client.session.prompt({
      sessionID: sessionId,
      noReply: true,
      parts: [{ type: "text", text: "OnMyAgent e2e context injection" }],
    });
    const msgs = await client.session.messages({ sessionID: sessionId, limit: 50 });
    assert.ok(Array.isArray(msgs));
    return { count: msgs.length };
  ...
  await step("event.subscribe", async () => {
    ...
    // Trigger events.
    await client.session.update({ sessionID: sessionId, title: "OnMyAgent e2e (updated)" });
