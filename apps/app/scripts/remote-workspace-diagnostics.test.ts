...
function workspace(overrides: Partial<WorkspaceInfo> = {}): WorkspaceInfo {
  return {
    ...
    onmyagentHostUrl: "https://worker.example.com/w/ws_remote",
    onmyagentToken: "ow-token",
    ...overrides,
  };
}
...
describe("resolveRemoteWorkspaceConnectionTarget", () => {
  test("builds a host-scoped OnMyAgent target from saved worker credentials", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        onmyagentHostUrl: "https://worker.example.com",
        onmyagentWorkspaceId: "ws_remote",
      }),
    );
  ...
  test("uses base URL workspace id and fallback client token when host fields are absent", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        baseUrl: "https://fallback.example.com/workspace/ws_from_base",
        onmyagentClientToken: "legacy-client-token",
        onmyagentHostUrl: "  ",
        onmyagentToken: "",
      }),
    );
    ...
    expect(target.ok).toBe(true);
    if (!target.ok) return;
    expect(target.target.baseUrl).toBe("https://fallback.example.com");
    expect(target.target.workspaceId).toBe("ws_from_base");
    expect(target.target.token).toBe("legacy-client-token");
  });
  ...
  test("does not run OnMyAgent probes against non-OnMyAgent remote workspaces", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        onmyagentHostUrl: "",
    ...
    if (target.ok) return;
    expect(target.state.status).toBe("error");
    expect(target.state.message).toContain("OnMyAgent remote workers");
  });

  test("does not run OnMyAgent probes against stale OnMyAgent fields on non-OnMyAgent remotes", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        onmyagentHostUrl: "https://worker.example.com/w/ws_remote",
        onmyagentToken: "owt_secret",
        baseUrl: "https://opencode.example.com",
      }),
    );
...
describe("testRemoteWorkspaceConnection", () => {
  ...
  test("reports a missing token after proving the worker endpoint is reachable", async () => {
    const result = await testRemoteWorkspaceConnection(workspace({ onmyagentToken: "" }), {
      createClient: () => client(),
    });
    ...
    expect(result.state.message).toContain("Upgrade the OnMyAgent host");
    expect(result.state.message).toContain("team@onmyagentlabs.com");
  });
  ...
  test("uses fallback OnMyAgent tokens saved on older workspace records", async () => {
...
describe("remote diagnostic identity", () => {
  ...
  test("changes when connection credentials change", () => {
    const before = getRemoteWorkspaceConnectionKey(workspace({ onmyagentToken: "old-token" }));
    const after = getRemoteWorkspaceConnectionKey(workspace({ onmyagentToken: "new-token" }));
