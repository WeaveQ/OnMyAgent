...
const opencodeGithubRepo = (() => {
  const raw =
    ...
    process.env.ONMYAGENT_OPENCODE_GITHUB_REPO?.trim() ||
    "anomalyco/opencode";
...
// onmyagent-server paths
const onmyagentServerBaseName = "onmyagent-server";
const onmyagentServerName = isWindowsTarget ? `${onmyagentServerBaseName}.exe` : onmyagentServerBaseName;
const onmyagentServerPath = join(sidecarDir, onmyagentServerName);
const onmyagentServerBuildName = bunTarget
  ? `${onmyagentServerBaseName}-${bunTarget}${bunTarget.includes("windows") ? ".exe" : ""}`
  : onmyagentServerName;
const onmyagentServerBuildPath = join(sidecarDir, onmyagentServerBuildName);
const onmyagentServerTargetTriple = resolvedTargetTriple;
const onmyagentServerTargetName = onmyagentServerTargetTriple
  ? `${onmyagentServerBaseName}-${onmyagentServerTargetTriple}${onmyagentServerTargetTriple.includes("windows") ? ".exe" : ""}`
  : null;
const onmyagentServerTargetPath = onmyagentServerTargetName ? join(sidecarDir, onmyagentServerTargetName) : null;

const onmyagentServerDir = resolve(__dirname, "..", "..", "server");
...
// orchestrator paths
const orchestratorBaseName = "onmyagent-orchestrator";
...
const findLocalOpencodeBinary = () => {
  ...
  const pathResult = spawnSync("command", ["-v", "opencode"], {
    encoding: "utf8",
    shell: true,
  });
  const pathCandidate = pathResult.status === 0 ? pathResult.stdout.trim() : "";
  if (pathCandidate) candidates.unshift(pathCandidate);

  for (const candidate of [...new Set(candidates)]) {
    if (!candidate || !existsSync(candidate) || isStubBinary(candidate)) continue;
    const version = normalizeVersion(readBinaryVersion(candidate));
...
const copyLocalOpencodeSidecar = (localBinary, expectedVersion) => {
  ...
  preparedOpencodeVersion = localBinary.version;
  return true;
};
...
const onmyagentServerVersion = (() => {
  try {
    const raw = readFileSync(resolve(onmyagentServerDir, "package.json"), "utf8");
