#!/usr/bin/env python3
"""
Expert Validator - Validates an expert package against the OnMyAgent specification.
...
def validate_file_structure(expert_dir, expert_type, result):
    ...
    # .expert-plugin/plugin.json must exist
    pj = expert_dir / '.expert-plugin' / 'plugin.json'
    if not pj.exists():
        result.error(".expert-plugin/plugin.json not found")
    ...
    # agents/, skills/, bin/ must be at root, not inside .expert-plugin/
    cb_dir = expert_dir / '.expert-plugin'
    for subdir in ('agents', 'skills', 'bin', 'avatars'):
        if (cb_dir / subdir).exists():
            result.error(f"{subdir}/ found inside .expert-plugin/ — must be at plugin root")
...
def validate_expert(expert_path):
    ...
    default_plugins_dir = Path.home() / '.onmyagent' / 'marketplaces' / 'my-experts'
    try:
        expert_dir.relative_to(default_plugins_dir)
    except ValueError:
        result.error(
            f"专家不在专家目录下，OnMyAgent 将无法检测到该专家。"
            f" 专家目录: {default_plugins_dir}"
        )

    # Load plugin.json
    pj_path = expert_dir / '.expert-plugin' / 'plugin.json'
