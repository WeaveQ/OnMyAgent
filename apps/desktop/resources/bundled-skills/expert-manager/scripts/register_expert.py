...
Only registered experts will be visible in the OnMyAgent expert center.

Usage:
    register_expert.py <expert-dir> [--marketplace-dir <marketplace-dir>] [--session-id <session-id>]

Examples:
    python3 register_expert.py ~/.onmyagent/marketplaces/my-experts/my-expert
    python3 register_expert.py ./my-expert --marketplace-dir ~/.onmyagent/marketplaces/my-experts
...
def find_plugin_json(expert_dir: Path) -> Optional[Path]:
    ...
    for meta_dir in ['.expert-plugin', '.onmyagent-plugin']:
...
def validate_expert_completeness(expert_dir: Path) -> Tuple[bool, List[str]]:
    ...
    if not plugin_json_path:
        errors.append("Missing plugin.json (checked .expert-plugin/ and .onmyagent-plugin/)")
        return False, errors
...
def get_marketplace_dir(expert_dir: Path, explicit_marketplace_dir: Optional[str]) -> Path:
    ...
    # Infer from expert_dir path: .../marketplaces/my-experts/<name>
    # -> marketplace dir is .../marketplaces/my-experts/
    parts = expert_dir.parts
    try:
        marketplace_idx = len(parts) - 2  # parent of expert dir should be "my-experts"
        if parts[marketplace_idx] == 'my-experts':
            return Path(*parts[:marketplace_idx + 1])
    except (IndexError, ValueError):
        pass

    # Fallback to default
    return Path.home() / '.onmyagent' / 'marketplaces' / 'my-experts'
...
def register_expert(expert_dir: Path, marketplace_dir: Path, session_id: Optional[str] = None) -> bool:
    ...
    manifest_path = marketplace_dir / '.expert-plugin' / 'marketplace.json'
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExamples:")
        print("  python3 register_expert.py ~/.onmyagent/marketplaces/my-experts/my-expert")
        print("  python3 register_expert.py ./my-expert --marketplace-dir ~/.onmyagent/marketplaces/my-experts")
        sys.exit(1)
    ...
    if success:
        print(f"\n🎉 Expert '{expert_dir.name}' is now registered and visible in OnMyAgent!")
    else:
        print(f"\n❌ Registration failed")
        sys.exit(1)
