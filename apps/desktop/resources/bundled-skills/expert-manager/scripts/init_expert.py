...
```
~/.onmyagent/marketplaces/my-experts/%(name)s/
```

然后运行注册命令使其在 OnMyAgent 中可见：

```bash
python3 scripts/register_expert.py ~/.onmyagent/marketplaces/my-experts/%(name)s/
```
...
def init_agent(expert_dir, name):
    ...
    plugin_dir = expert_dir / '.expert-plugin'
    plugin_dir.mkdir()
    (plugin_dir / 'plugin.json').write_text(
        AGENT_PLUGIN_JSON % {'name': name, 'agent_name': agent_name},
        encoding='utf-8'
    )
    print("  ✅ .expert-plugin/plugin.json")
...
def main():
    ...
    # 硬检查：--path 必须是专家目录
    expected_path = Path.home() / '.onmyagent' / 'marketplaces' / 'my-experts'
    resolved_path = Path(output_path).expanduser().resolve()
    if resolved_path != expected_path:
        print(f"❌ Error: --path 必须为专家目录: {expected_path}")
        print(f"   当前传入: {resolved_path}")
        print(f"   非专家目录下创建的专家无法被 OnMyAgent 检测到。")
        sys.exit(1)
