#!/usr/bin/env python3
"""Render a spreadsheet workbook to PDF and per-page PNG files for visual QA."""
...
SUPPORTED_SUFFIXES = {".xlsx", ".xls", ".xlsm", ".ods"}
...
def render(source: Path, output_dir: Path, dpi: int, emit_pdf: bool) -> dict[str, object]:
    if source.suffix.lower() not in SUPPORTED_SUFFIXES or not source.is_file():
        ...
        raise ValueError(f"Input must be an existing workbook ({supported}): {source}")
    ...
    with tempfile.TemporaryDirectory(prefix="onmyagent-sheet-render-") as temp:
        temp_dir = Path(temp)
