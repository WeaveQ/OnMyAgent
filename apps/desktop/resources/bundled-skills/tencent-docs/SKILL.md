...
**前提条件**：本 Skill 依赖腾讯文档连接器，必须先配置连接器并完成 OAuth 授权后才能使用。连接器未配置或未授权时，所有工具调用都会失败。

### 未授权时的处理

当用户尝试使用腾讯文档功能，但连接器不可用时，按以下情况处理：

| 情况 | 判断依据 | AI 应引导用户 |
|------|---------|--------------|
| 连接器未配置 | 调用工具时返回连接器不存在 / 未找到等错误 | "检测到尚未配置腾讯文档连接器，请先打开「市场 → 连接器」，在推荐区找到「腾讯文档」卡片并完成授权。" |
| 连接器已配置但未授权 | 调用工具时返回 401 未授权 / token 不存在等错误 | "腾讯文档尚未授权，请打开「市场 → 连接器」中的「腾讯文档」卡片，点击「授权」并完成微信扫码。" |
| 授权已过期 | 调用工具时返回错误码 `400006`（Token 鉴权失败） | "腾讯文档授权已过期，请重新扫码授权。" |

> 授权流程由连接器自动处理（OAuth 2.1 + PKCE），用户只需扫码确认即可，无需手动配置 token。
...
├── aipage_pack.js                  # 本地 HTML 打包成 .aipage（纯 Node.js，无外部依赖）
...
client.callTool({ name: "<工具名>", arguments: { ... } })

// PPT 精细编辑工具（slide-mcp 服务，slide_* 前缀）
client.callTool({ name: "slide_<工具名>", arguments: { ... } })
```

> ⚠️ 参考文档中的参数说明应与 MCP 工具 Schema 保持一致。如有冲突，以 `client.listTools()` 返回的 Schema 为准。
...
- **本地 HTML 一键上云**：`node aipage_pack.js` 打包成 .aipage → 调用 `manage.pre_import` 获取上传链接 → PUT 上传到 COS → `manage.async_import` 触发 → `manage.import_progress` 轮询，详见 `references/aipage_references.md`。
- **OCR 图片识别**：`ocr.extract` 提取文字 / `ocr.toword` 图片转在线文档 / `ocr.toexcel` 图片转在线表格；本地图片需先转 base64 再通过 `image_base64` 参数传入，公网 URL 图片直接用 `image_url` 参数，详见 `references/ocr_references.md`
...
- **用户有本地文件保存/沉淀/落盘**：一律使用 `manage.pre_import` → PUT 上传 COS → `manage.async_import` → `manage.import_progress` 统一上传通路，保留原文件结构，不要用 `create_*` 工具重新生成内容；文件格式是否支持由后端判定，收到"不支持"错误时再降级到其他通路
...
- **OCR 图片识别**：`image_url`（公网 URL）与 `image_base64` 严格二选一；本地图片需先转 base64 再通过 `image_base64` 参数传入，详见 `references/ocr_references.md`
...
| 错误码     | 错误类型           | 解决方案                                                                                                                                                                 |
| ---------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **400006** | **Token 鉴权失败** | Token 鉴权失败，重新发起连接器 OAuth 授权流程                                                                                                                            |
...
2. **确认工具可用**：`client.listTools()` 查看可用工具列表及参数定义
3. **确认授权状态**：如有鉴权相关错误，重新发起连接器 OAuth 授权流程
...
```
client.callTool({ name: "check_skill_update", arguments: { version: "<version>" } })
```
JSON 格式数据返回，返回参数示例：
