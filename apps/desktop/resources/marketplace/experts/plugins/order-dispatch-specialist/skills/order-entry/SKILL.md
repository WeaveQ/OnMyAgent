...
- 当前工作目录是专家会话根目录；本能力的固定目录为 `物流单/`。
- 开始处理前先创建 `物流单/`，数据源固定为 `物流单/waybill-data.json`，过程产物固定在 `物流单/.process/`，正式产物直接放在 `物流单/`。
- 禁止把本能力文件写到会话根或其他能力目录。交付链接必须带目录，例如 `artifact:物流单/物流单_XX_最终版.pdf`。
...
3. **维护单一数据源**：按 `references/waybill-data-protocol.md` 更新 `物流单/waybill-data.json`。每个字段保留来源与置信度；冲突放入 `conflicts`，低置信度字段放入 `lowConfidenceFields`，禁止直接覆盖旧值掩盖冲突。
4. **生成 HTML 初稿（过程产物，仅 HTML）**：模板确认后调用
   `python3 <Skill根目录>/scripts/generate_waybill.py --input 物流单/waybill-data.json --output-dir 物流单 --mode preview`
   **preview 只生成过程 HTML**，写入 `物流单/.process/`；**不启动浏览器，不生成 PDF/XLSX**。结果 PDF/Excel 仅在用户选择格式后的 export 阶段生成。用户无指定模板且制作物流单时，脚本必须读取 `assets/logistics-waybill-template.html`，只替换有 `data-field`/`data-check` 标识的值；禁止增删区块、重排字段、改变合并单元格、配色、字体、边框、联次或自行重做版式。用户有模板时才通过 `--template` 使用忠实复刻的用户模板。不要用普通 Markdown 表格代替效果图。
...
8. **手动编辑字段**：预览内支持“编辑字段 → 保存修改”。保存后**预览 DOM 直接保留已改数据**（可再次编辑），界面只提示“已保存”，**禁止**向用户展示 JSON / 补丁代码。客户端会把补丁**直接写入** `物流单/waybill-data.json`，**不会**为此再让你重跑 preview/show_widget（避免覆盖用户刚改的预览）。你在后续对话/export 时读最新 JSON 即可；若用户明确要求“按最新数据刷新预览”，再执行
   `python3 <Skill根目录>/scripts/generate_waybill.py --input 物流单/waybill-data.json --output-dir 物流单 --mode preview`
...
12. **交付产物（强制表格）**：过程 HTML 不提供用户链接。结果 PDF/XLSX 必须用两列表格交付，不得自由发挥：
    ...
    | 文件 | 操作 |
    | --- | --- |
    | <脚本返回的实际文件名.pdf> | [查看](artifact:物流单/<实际文件名.pdf>) |
    | <脚本返回的实际文件名.xlsx> | [查看](artifact:物流单/<实际文件名.xlsx>) |
    ```
...
- `assets/` 必须相对当前 Skill 根目录解析，不得去会话工作区搜索。若运行时未提供 Skill 根目录，使用安装路径 `~/.onmyagent/marketplaces/experts/order-dispatch-specialist/skills/order-entry/assets/logistics-waybill-template.html`。两个位置都不可读时停止生成并报告“专家模板安装异常”，禁止绕过模板继续制作。
...
- **过程产物**只放 `物流单/.process/`；**结果产物** PDF/XLSX 直接放 `物流单/`（与 `waybill-data.json` 同级）。**禁止**再建 `output/` 子目录。
...
- 禁止在正文输出 `show_widget` 围栏；客户端会直接解析脚本返回的完整 `inlineWidget` JSON，不得自己重画简化版；也不得调用 `render_visual` 等可视化工具二次渲染。
