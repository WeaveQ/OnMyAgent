...
- 当前工作目录是专家会话根目录；固定使用 `报价/`。
- `quote-request.json`、`.process/`、报价文件和话术全部放入 `报价/`，禁止写入会话根或其他能力目录。
- 交付链接必须带目录，例如 `artifact:报价/报价方案_XX.xlsx`。
...
9. **逐轮生成三档预览 HTML**：按 `references/data-protocol.md` 维护 `quote-request.json`，每次收到信息后跑 preview：
   ```bash
   python3 <Skill根目录>/scripts/build_quote_artifacts.py --input 报价/quote-request.json --output-dir 报价 --mode preview
   ```
...
10. **导出前格式确认（必须）+ 导出**：用户确认报价后**不要**立刻 export。先用选择题请用户点选其一：生成 Excel 和 Word / 只生成 Excel / 只生成 Word / 先不生成。只有用户明确选择前三项之一后才运行：
    ```bash
    python3 <Skill根目录>/scripts/build_quote_artifacts.py --input 报价/quote-request.json --output-dir 报价 --mode export
    ```
...
11. **交付产物（强制表格）**：过程 HTML 不提供用户链接。结果 Excel/Word 必须用两列表格交付，不得自由发挥：
    ...
    | 文件 | 操作 |
    | --- | --- |
    | <脚本返回的实际文件名.xlsx> | [查看](artifact:报价/<实际文件名.xlsx>) |
    | <脚本返回的实际文件名.docx> | [查看](artifact:报价/<实际文件名.docx>) |
    ```
