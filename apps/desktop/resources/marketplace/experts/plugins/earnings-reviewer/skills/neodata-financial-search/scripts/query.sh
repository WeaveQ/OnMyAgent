...
LEGACY_TOKEN_FILE="$HOME/.onmyagent/.neodata_token"
...
if [[ -n "$DATA_TYPE" ]]; then
    BODY=$(printf '{"query": "%s", "channel": "neodata", "sub_channel": "onmyagent", "data_type": "%s"}' "$QUERY" "$DATA_TYPE")
else
    BODY=$(printf '{"query": "%s", "channel": "neodata", "sub_channel": "onmyagent"}' "$QUERY")
fi
