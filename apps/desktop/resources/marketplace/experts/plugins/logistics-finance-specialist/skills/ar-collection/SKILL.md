...
- 当前工作目录是专家会话根目录；固定使用 `回款催收/`。
- 应收数据、`.process/`、台账、话术和自动化提案全部放入 `回款催收/`。
- 交付链接必须带目录，例如 `artifact:回款催收/应收台账_XX.xlsx`。

## 标准作业流程

1. **立规则**：账期起算（对账确认日 / 开票日 / 月结固定日）、是否接受承兑。见 `references/ar-ledger.md`。  
2. **维护单一数据源**：更新 `回款催收/ar-ledger.json`（结构见 `references/data-protocol.md`）。禁止手改 Excel/CSV 当主数据。
3. **算账龄与节点**：`references/aging-nodes.md` 默认 D-7 / due / +3 / +15。  
4. **逐轮生成催收看板预览 HTML**：每次更新 ar-ledger.json 后跑 preview：
   ```bash
   python3 <Skill根目录>/scripts/build_ar_artifacts.py --input 回款催收/ar-ledger.json --output-dir 回款催收 --mode preview
   ```
...
6. **导出前格式确认（必须）+ 导出**：用户确认台账后**不要**立刻 export。先用选择题请用户点选其一：生成 Excel 和 PDF / 只生成 Excel / 只生成 PDF / 先不生成。只有用户明确选择前三项之一后才运行：
   ```bash
   python3 <Skill根目录>/scripts/build_ar_artifacts.py --input 回款催收/ar-ledger.json --output-dir 回款催收 --mode export
   ```
...
7. **交付产物（强制表格）**：过程 HTML 不提供用户链接。结果 Excel/PDF 必须用两列表格交付，不得自由发挥：
    ...
    | 文件 | 操作 |
    | --- | --- |
    | <脚本返回的实际文件名.xlsx> | [查看](artifact:回款催收/<实际文件名.xlsx>) |
    | <脚本返回的实际文件名.pdf> | [查看](artifact:回款催收/<实际文件名.pdf>) |
    ```
