...
4. **生成并内嵌三联效果图（仅过程 HTML）**：维护 `output/waybill-data.json` 后调用 Skill 的 `scripts/generate_waybill.py --mode preview`，在 `output/.process/` 生成白/红/黄三个独立 HTML。**此阶段不生成 PDF/Excel、不启动浏览器。** 脚本成功后，用户可见回复中只放：一句状态说明 + 完整 `show_widget`（脚本 `inlineWidget` 原样）+ 三个 `preview:` 放大链接 + 合并追问。
