...
7. **逐轮内嵌三联效果图**：每次收到用户补充后，原位更新同一份 JSON 并重跑 preview 命令。preview **只写一份** `.process/…_当前预览.html`，会话内 Tab 切换联次仅改纸色/联次名（不复制三份 HTML，显著更快）。把脚本返回的完整 `inlineWidget` JSON **原样**放入 `show_widget` 围栏；默认白色存根联；围栏后给一个 `[放大查看](preview:.process/脚本返回的实际预览文件名.html)`。**禁止**把脚本 stdout、HTML 源码、半截 JSON 贴进用户可见正文。
8. **手动编辑字段**：预览内支持“编辑字段 → 保存修改”。保存后**预览图直接保留已改数据**，界面只提示“已保存”，**禁止**向用户展示 JSON / `waybill-patch` 围栏 / 补丁原文。客户端会把补丁作为用户消息附带 `waybill-patch` 发给你；收到后必须立刻执行
   `python3 <Skill根目录>/scripts/generate_waybill.py --input waybill-data.json --output-dir . --mode preview --patch <临时patch.json> --write-input`
   （或先把 patch 写入 JSON 再 preview）。脚本会回写 `waybill-data.json`、刷新 `.process` 预览，并在数据指纹变化时**自动删除**旧 PDF/XLSX。回复用户时只放简短确认 + 完整 `show_widget` + 必要追问，**禁止**复述补丁代码。
...
12. **交付产物（强制表格）**：三个 HTML 只给 `preview:.process/...`。结果 PDF/XLSX 必须用两列表格交付，不得自由发挥：

    ```markdown
    | 文件 | 操作 |
    | --- | --- |
    | <脚本返回的实际文件名.pdf> | [在文件夹中显示](artifact:<实际文件名.pdf>) |
    | <脚本返回的实际文件名.xlsx> | [在文件夹中显示](artifact:<实际文件名.xlsx>) |
    ```

    - 操作列文案固定为 **「在文件夹中显示」**（不要写「打开 PDF/Excel/下载」）。
    - 链接协议固定 `artifact:...`，点击 = 在文件管理器中定位该文件。
