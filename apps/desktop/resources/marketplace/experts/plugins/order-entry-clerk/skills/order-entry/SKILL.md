...
7. **逐轮内嵌三联效果图**：每次收到用户补充后，原位更新同一份 JSON 并重跑 preview 命令。preview **只写一份** `output/.process/…_当前预览.html`，会话内 Tab 切换联次仅改纸色/联次名（不复制三份 HTML，显著更快）。把脚本返回的完整 `inlineWidget` JSON **原样**放入 `show_widget` 围栏；默认白色存根联；围栏后给一个 `[放大查看](preview:output/.process/脚本返回的实际预览文件名.html)`。**禁止**把脚本 stdout、HTML 源码、半截 JSON 贴进用户可见正文。
...
12. **交付产物（强制表格）**：三个 HTML 只给 `preview:output/.process/...`。结果 PDF/XLSX 必须用两列表格交付，不得自由发挥：

    ```markdown
    | 文件 | 操作 |
    | --- | --- |
    | <脚本返回的实际文件名.pdf> | [在文件夹中显示](artifact:output/<实际文件名.pdf>) |
    | <脚本返回的实际文件名.xlsx> | [在文件夹中显示](artifact:output/<实际文件名.xlsx>) |
    ```

    - 操作列文案固定为 **「在文件夹中显示」**（不要写「打开 PDF/Excel/下载」）。
    - 链接协议固定 `artifact:output/...`，点击 = 在文件管理器中定位该文件。
