...
9. **确定性三联导出**：确认后运行同一脚本的 `--mode export`。状态为 `pending_dispatch` 时，白/红/黄三联分别生成带“待派车确认稿”后缀的 PDF/XLSX；状态为 `final` 时，三联分别生成带“最终版”后缀的 PDF/XLSX。每个 Excel 固定有 `物流单` 与 `字段数据` 两个工作表。脚本失败或任一文件不存在时如实说明，禁止宣称已生成。导出成功后必须把这次 export 返回的完整 `inlineWidget` 再次放入 `show_widget` 围栏；其中包含三联导出映射，客户端会让用户在当前 Tab 的右上角菜单导出对应 PDF 或 Excel。
10. **交付产物**：三个 HTML 只给各自 `preview:` 链接；三份 PDF 与三份 XLSX 分别给 `[打开产物](artifact:output/脚本返回的实际文件名)`，点击行为是在系统文件管理器中定位文件。不得把内部 JSON 作为用户主产物，也不得使用 `[下载](output/...)`、普通相对链接、`file://` 或 `sandbox:` 链接。
