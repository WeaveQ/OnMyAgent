...
| `应收台账_*.xlsx` | 结果产物：台账 Excel（默认；有风险行标红） |
| `应收台账_*.csv` | 结果产物：台账 CSV 侧车（兼容导入） |
...
字段含义见 `ar-ledger.md`。Agent **只维护本 JSON**；报表/Excel/CSV 由脚本从 JSON 生成，禁止手改表格造成不一致。
...
| `armed` | 用户确认创建/更新定时提醒 | 写入 automations proposals + 结果 Excel/CSV/话术包 |
