...
6. **用户确认后 export**：
   ...
   ```
   产出 **Excel（默认，`riskFlags`/超期行浅红底）** + CSV 侧车、话术包、每日看板 proposal，并为每笔未结清应收生成“下一个 D-7 / 到期 / +3 / +15 节点”的 once proposal。
    **交付产物（强制表格）**：过程看板（`.process/`）不提供用户链接。结果 Excel/CSV/话术必须用两列表格交付，不得自由发挥：
...
- 结果产物（应收台账 Excel/CSV/催收话术）必须用两列表格 + `artifact:` 链接交付，操作列固定「查看」；过程产物（`.process/`）不提供用户链接，禁止 `file://` / `sandbox:` / 普通相对链接。
