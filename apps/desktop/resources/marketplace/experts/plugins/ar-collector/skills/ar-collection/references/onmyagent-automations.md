...
2. **用户确认**：回复「确认创建」/「确认创建定时任务」时，**OnMyAgent 专家宿主会自动**读取提案并调用 `createAutomation` 真正创建定时任务（同标题已存在则跳过）。  
3. 若宿主创建失败，提示用户在侧栏 **定时任务** 手动新建，或检查权限/审批。  
4. 用户可见回复只保留：**任务列表表** + 引导说「确认创建」，不要贴整段 JSON。
