...
1. **export 后**：脚本写入每日看板 proposal，并按每笔未结清应收当前日期选择下一个 D-7 / 到期 / +3 / +15 节点生成 once proposal；超过 +15 且没有未来节点的记录由每日看板持续跟进。
...
| 应收催收·客户·票号·下一节点 | 下一节点 09:00（一次） | 复核余额后生成话术并提醒负责人，不自动发送 |
```
