...
- 当前工作目录是专家会话根目录；固定使用 `挂靠车管理/`。
- 台账、`.process/`、导出文件、话术和自动化提案全部放入 `挂靠车管理/`。
- 交付链接必须带目录，例如 `artifact:挂靠车管理/挂靠车台账_XX.xlsx`。

## 标准作业流程

1. **收资料** → 2. **维护 `挂靠车管理/fleet-ledger.json`**（`ledger-fields.md` + `data-protocol.md`）
3. **算提醒**（`expiry-alerts.md`）→ 4. **扫高风险**（`risk-and-scripts.md`）
5. **逐轮生成合规看板预览 HTML**：每次更新 fleet-ledger.json 后跑 preview：
   ```bash
   python3 <Skill根目录>/scripts/build_fleet_artifacts.py --input 挂靠车管理/fleet-ledger.json --output-dir 挂靠车管理 --mode preview
   ```
...
6. **导出前格式确认（必须）+ 导出** + **定时任务**（`onmyagent-automations.md`）：用户确认台账后**不要**立刻 export。先用选择题请用户点选其一：生成 Excel 和 PDF / 只生成 Excel / 只生成 PDF / 先不生成。只有用户明确选择前三项之一后才运行：
   ```bash
   python3 <Skill根目录>/scripts/build_fleet_artifacts.py --input 挂靠车管理/fleet-ledger.json --output-dir 挂靠车管理 --mode export
   ```
...
7. **交付产物（强制表格）**：过程 HTML 不提供用户链接。结果 Excel/PDF 必须用两列表格交付，不得自由发挥：
    ...
    | 文件 | 操作 |
    | --- | --- |
    | <脚本返回的实际文件名.xlsx> | [查看](artifact:挂靠车管理/<实际文件名.xlsx>) |
    | <脚本返回的实际文件名.pdf> | [查看](artifact:挂靠车管理/<实际文件名.pdf>) |
    ```
...
- **禁止未确认创建定时任务**。
- 只在 `挂靠车管理/` 落文件，禁止写入会话根或增加 `output/`。
- 合规看板必须通过 preview HTML 卡片展示，**禁止只输出文字/表格分析而不跑 preview**；未跑 preview 不得声称已完成合规核查。
- 过程 HTML（`.process/fleet-preview.html`）只通过脚本返回的 `inlineWidget` 让客户端实时渲染，禁止 `cat`/读取 HTML 源码到对话、禁止 `file://`/浏览器/`preview:` 打开；禁止在正文输出 `show_widget` 围栏或半截 JSON；也不得调用 `render_visual` 等可视化工具二次渲染。
