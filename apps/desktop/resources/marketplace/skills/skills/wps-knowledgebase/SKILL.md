...
version: 2.0.1
description: >-
  WPS/zhishi（金山知识库）云端知识库：列表/详情、文件树、智能问答（ask SSE）、分享链接、创建关闭与文件 CRUD、导入上传与格式转换（OTL 下载限制见正文）。 当用户用中文只说「知识库」「我的知识库」「在知识库里查/搜/问」「从知识库找答案」「知识库问答」「zhishi」「kwiki」或英文 knowledge base / ask，且意图是云端 Skills Hub 里的内容而非仅搜索本地仓库文件时，必须使用本 skill 并用 `kwiki-cli kwiki <tool>` 执行。 亦适用于终端、CI、脚本自动化及 access_token / `X_KWIKI_AUTH` 等多账号鉴权场景。
display_name: "WPS 知识库"
display_name_en: "WPS Knowledge Base"
description_zh: "WPS/zhishi（金山知识库）云端知识库 CLI：支持知识库列表/详情、文件树浏览、智能问答（RAG）、分享链接、创建关闭与文件 CRUD、导入上传与格式转换。通过 kwiki-cli 命令行工具管理你的云端知识库。"
description_en: "WPS/zhishi cloud knowledge base CLI: list/detail, file tree browsing, smart Q&A (RAG), share links, create/close and file CRUD, import/upload and format conversion. Manage your cloud knowledge base via kwiki-cli."
visibility: "public"
---
...
本压缩包包含 `kwiki-cli.json` 清单文件，AI 助手（如 OpenClaw）可自动识别并执行安装。将本 zip 交给 AI 助手，告诉它"帮我安装"即可。

清单入口：`kwiki-cli.json` → 执行 `node install.js`
...
安装脚本会自动检测当前平台，从 CDN 下载对应的二进制，通过 `npm install -g` 安装 CLI，并将 Skill 文件复制到本地 Agent 目录。渠道标识会自动写入 `~/.kwiki-cli/config.json`。

## 使用

```bash
# 登录（首次使用）
kwiki-cli auth login

# 查看知识库列表
kwiki-cli kwiki knowledge-view-list
...
**限制**：**OTL 智能文档**（`doc_origin_type` 为 `otl` / `doc_type` 为 `o` 等）**不支持**通过本接口下载；调用 `file-download`（含 `file_base64`、`download_link`）会返回业务错误（如 `400408000`）。需取内容时请改用其它导出或转换能力，勿对 OTL 走 `file-download`。
...
| 下载知识库文件到本地 | 推荐 `file_base64` 模式，OTL 智能文档不支持下载 | `file-download` |
