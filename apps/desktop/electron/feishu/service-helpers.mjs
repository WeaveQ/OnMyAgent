...
export function splitTextForFeishu(text, maxLength = 7800) {
  ...
  while (rest.length > limit) {
    ...
    if (
      cut === 0
      && rest.length >= 2
      && isHighSurrogate(rest.charCodeAt(0))
      && isLowSurrogate(rest.charCodeAt(1))
    ) {
      cut = 2;
    }
...
export function normalizePromptMode(value) {
  return String(value ?? "raw").trim().toLowerCase() === "debug" ? "debug" : "raw";
}
...
export function resolveAgentAlias(agents, target) {
  ...
  return normalized ? agents.find((agent) => agentAliases(agent).includes(normalized)) ?? null : null;
}
...
export function scopedFeishuRuntimeAgent(agent, event) {
  const scopeHash = stableHash(`${event.accountId}\n${event.chatId}\n${agent.provider}\n${agent.id}`);
  return {
    ...agent,
    id: `${safeSegment(agent.id)}-feishu-${scopeHash}`,
    name: agent.name ? `${agent.name} · Feishu` : `${agent.provider} · Feishu`,
  };
}
