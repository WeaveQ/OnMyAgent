...
contextBridge.exposeInMainWorld("__ONMYAGENT_ELECTRON__", {
  ...
  officeCli: {
    ...
    onStatus(callback) {
      ...
      ipcRenderer.on("onmyagent:officecli:status", handler);
      return () => {
        ipcRenderer.removeListener("onmyagent:officecli:status", handler);
      };
    },
  },
  larkCli: {
    onProgress(callback) {
      const handler = (_event, progress) => callback(progress);
      ipcRenderer.on("onmyagent:lark-cli:progress", handler);
      return () => {
        ipcRenderer.removeListener("onmyagent:lark-cli:progress", handler);
      };
    },
    onStatus(callback) {
      const handler = (_event, status) => callback(status);
      ipcRenderer.on("onmyagent:lark-cli:status", handler);
      return () => {
