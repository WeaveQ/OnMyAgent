...
Desktop OpenCode e2e：`apps/desktop/electron/e2e/*.e2e.test.mjs`（编入 `test:runtime`；默认无 live 模型 / 无 Electron 窗）。覆盖 knowledge 插件加载与 vault 检索/CRUD、工作区 `/file` 读写、sandbox HOME 隔离 + slash 核心技能、skill-creator 写入 `profiles/local/config/skills`、以及 Task Supervisor runtime API / 主栏 Personal persist 隔离 + orphan reconcile。`createRuntimeManager` idle/`inProcess` 快照由 `runtime-engine-state.test.mjs` 覆盖，不在 e2e 里空转 factory default。可选 `OPENCODE_E2E_LIVE_MODEL=1` 跑免费模型 prompt。
