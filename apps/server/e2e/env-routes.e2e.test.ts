...
function hostAuth() {
  return { "x-onmyagent-host-token": HOST_TOKEN, "content-type": "application/json" };
}

beforeEach(() => {
  const dir = mkdtempSync(join(tmpdir(), "onmyagent-env-routes-"));
  dirs.push(dir);
  // Redirect the shared env.json path into a throwaway dir so the test never
  // touches the developer's real ~/.config/onmyagent/env.json.
  process.env.ONMYAGENT_ENV_STORE = join(dir, "env.json");
  process.env.ONMYAGENT_TOKEN_STORE = join(dir, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorEnvStore === undefined) {
    delete process.env.ONMYAGENT_ENV_STORE;
  } else {
    process.env.ONMYAGENT_ENV_STORE = priorEnvStore;
  }
  if (priorTokenStore === undefined) {
    delete process.env.ONMYAGENT_TOKEN_STORE;
  } else {
...
describe("env routes", () => {
  ...
  test("invalid env store returns 409 instead of overwriting on PUT", async () => {
    writeFileSync(process.env.ONMYAGENT_ENV_STORE!, "{ this is not json");
    const { base } = await boot();
  ...
  test("PUT rejects reserved keys with 400", async () => {
    ...
    const put = await fetch(`${base}/env`, {
      method: "PUT",
      headers: hostAuth(),
      body: JSON.stringify({ key: "ONMYAGENT_TOKEN", value: "x" }),
    });
    expect(put.status).toBe(400);
    const body = (await put.json()) as { code: string; message: string };
    expect(body.code).toBe("reserved_env_key");
    expect(body.message).toBe("Environment variable name is reserved for OnMyAgent internals");
    expect(body.message).not.toContain("ONMYAGENT_TOKEN");
  });
