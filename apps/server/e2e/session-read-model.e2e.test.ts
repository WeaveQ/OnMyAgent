...
async function createWorkspaceRoot(folderName?: string) {
  const root = await mkdtemp(join(tmpdir(), "onmyagent-session-read-"));
...
describe("workspace session read APIs", () => {
  test("lists sessions and returns session details, messages, and snapshot", async () => {
    ...
    const base = `http://127.0.0.1:${onmyagent.server.port}`;
  ...
  test("accepts guest-side rem_ workspace aliases for session reads", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${onmyagent.server.port}/workspace/rem_ws_1/sessions`, {
      headers: auth(onmyagent.token),
    });
  ...
  test("uses a session workspace directory override for all session reads", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    ...
    const base = `http://127.0.0.1:${onmyagent.server.port}`;
    const directory = encodeURIComponent(sessionWorkspaceRoot);

    await fetch(`${base}/workspace/ws_1/sessions?directory=${directory}`, {
      headers: auth(onmyagent.token),
    });
    await fetch(
      `${base}/workspace/ws_1/sessions/ses_1?directory=${directory}`,
      { headers: auth(onmyagent.token) },
    );
    await fetch(
      `${base}/workspace/ws_1/sessions/ses_1/messages?directory=${directory}`,
      { headers: auth(onmyagent.token) },
    );
    await fetch(
      `${base}/workspace/ws_1/sessions/ses_1/snapshot?directory=${directory}`,
      { headers: auth(onmyagent.token) },
    );

    const sessionRequests = mock.requests.filter((request) =>
      request.pathname.startsWith("/session"),
    );
    expect(sessionRequests.length).toBe(7);
    expect(
      sessionRequests.every(
        (request) => request.directory === sessionWorkspaceRoot,
      ),
    ).toBe(true);
  });
  ...
  test("encodes non-ASCII workspace directory headers for session reads", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${onmyagent.server.port}/workspace/ws_1/sessions`, {
      headers: auth(onmyagent.token),
    });
  ...
  test("encodes non-ASCII workspace directory headers for opencode proxy requests", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${onmyagent.server.port}/workspace/ws_1/opencode/session`, {
      headers: auth(onmyagent.token),
    });
  ...
  test("returns 404 when the upstream session is missing", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${onmyagent.server.port}/workspace/ws_1/sessions/ses_missing/snapshot`, {
      headers: auth(onmyagent.token),
    });
  ...
  test("acknowledges proxied session commands before upstream completion", async () => {
    ...
    const response = await Promise.race([
      fetch(`http://127.0.0.1:${onmyagent.server.port}/workspace/ws_1/opencode/session/ses_1/command`, {
        method: "POST",
        headers: { ...auth(onmyagent.token), "Content-Type": "application/json" },
