...
import { buildCommandContent } from "../src/services/commands.js";
import { startServer } from "../src/server.js";
import { buildSkillContent } from "../src/services/skills.js";
import type { ServerConfig } from "@onmyagent/types/server";
import { globalSkillsDir } from "../src/workspace/workspace-files.js";
import {
...
} from "../src/workspace/workspace-import-preview.js";

const tempDirs: string[] = [];
let originalGlobalSkillsDir: string | undefined;

beforeEach(async () => {
  const dir = await mkdtemp(join(tmpdir(), "onmyagent-global-skills-"));
  tempDirs.push(dir);
  originalGlobalSkillsDir = process.env.OPENCODE_GLOBAL_SKILLS_DIR;
  process.env.OPENCODE_GLOBAL_SKILLS_DIR = dir;
});
...
// Helper to get the relative path from workspace to global skill file
function globalSkillRelativePath(workspace: string, skillName: string, file = "SKILL.md"): string {
  return relative(workspace, join(globalSkillsDir(), skillName, file)).replaceAll("\\", "/");
}

// Helper to get the relative path to global skill directory (no SKILL.md suffix)
function globalSkillDirRelativePath(workspace: string, skillName: string): string {
  return relative(workspace, join(globalSkillsDir(), skillName)).replaceAll("\\", "/");
}

async function makeWorkspace(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "onmyagent-import-preview-"));
  tempDirs.push(dir);
...
async function silenceExpectedServerError<T>(run: () => Promise<T>): Promise<T> {
  ...
  console.error = (...args: unknown[]) => {
    if (args[0] === "[onmyagent-server] Unhandled error:") return;
    originalError(...args);
  };
  try {
    return await run();
...
async function waitForPendingApproval(baseUrl: string): Promise<string> {
  for (let attempt = 0; attempt < 50; attempt += 1) {
    const response = await fetch(`${baseUrl}/approvals`, {
      headers: { "X-OnMyAgent-Host-Token": "host-token" },
    });
    expect(response.status).toBe(200);
...
describe("workspace import preview", () => {
  test("summarizes workspace import changes without writing files", async () => {
    ...
    await writeFile(join(globalSkills, "demo", "SKILL.md"), "old skill\n", "utf8");
    ...
    expect(preview.changes.map((change) => [change.kind, change.action, change.path])).toEqual([
      ...
      ["onmyagent", "create", ".opencode/onmyagent.json"],
      ["skill", "update", globalSkillRelativePath(workspace, "demo")],
      ["skill", "create", globalSkillRelativePath(workspace, "new-skill")],
  ...
  test("marks identical skills and commands unchanged", async () => {
    ...
    await mkdir(join(globalSkills, "demo"), { recursive: true });
    await mkdir(join(workspace, ".opencode", "commands"), { recursive: true });
    await writeFile(join(globalSkills, "demo", "SKILL.md"), skillContent.content, "utf8");
    ...
    expect(preview.changes.map((change) => [change.kind, change.action, change.path])).toEqual([
      ["skill", "unchanged", globalSkillRelativePath(workspace, "demo")],
  ...
  test("replace preview treats empty sections as delete all", async () => {
    const workspace = await makeWorkspace();
    const globalSkills = globalSkillsDir();
    await mkdir(join(globalSkills, "old-skill"), { recursive: true });
    await mkdir(join(workspace, ".opencode", "commands"), { recursive: true });
    await mkdir(join(workspace, ".opencode", "agents"), { recursive: true });
    await writeFile(join(globalSkills, "old-skill", "SKILL.md"), "old skill\n", "utf8");
    ...
    expect(preview.changes.map((change) => [change.kind, change.action, change.path])).toEqual([
      ["skill", "delete", globalSkillDirRelativePath(workspace, "old-skill")],
  ...
  test("preview route returns public changes and no-op import does not audit", async () => {
    const workspace = await makeWorkspace();
    const dataDir = await mkdtemp(join(tmpdir(), "onmyagent-import-preview-data-"));
    ...
    try {
      ...
      const importResponse = await fetch(`${baseUrl}/workspace/workspace/import`, {
        method: "POST",
        headers,
        body: JSON.stringify({ ...JSON.parse(body), previewFingerprint: preview.fingerprint }),
      });
      expect(importResponse.status).toBe(200);
      const imported = await importResponse.json() as Record<string, unknown>;
      expect(imported.preview).toEqual(preview);
      expect(await readFile(auditLogPath("workspace"), "utf8")).toContain("Imported workspace config (update 1)");
    } finally {
      server.stop(true);
  ...
  test("import route writes changed items and records audit", async () => {
    ...
    try {
      ...
      expect(await readFile(join(globalSkillsDir(), "demo", "SKILL.md"), "utf8")).toContain("Demo skill");
  ...
  test("replace import route removes extra skills, commands, and portable files", async () => {
    const workspace = await makeWorkspace();
    // Remove auto-created files to test fresh import
    await rm(join(workspace, "opencode.jsonc"), { force: true });
    await rm(join(workspace, ".opencode", "agents", "onmyagent.md"), { force: true });

    // Set up skills in global directory
    const globalSkills = globalSkillsDir();
    await mkdir(join(globalSkills, "keep"), { recursive: true });
    await writeFile(
      join(globalSkills, "keep", "SKILL.md"),
      "---\nname: keep\ndescription: Keep skill\n---\nKeep content",
      "utf8"
    );
    await mkdir(join(globalSkills, "remove-me"), { recursive: true });
    await writeFile(
      join(globalSkills, "remove-me", "SKILL.md"),
      "---\nname: remove-me\ndescription: Remove skill\n---\nRemove content",
      "utf8"
    );

    // Set up commands in workspace directory
    await mkdir(join(workspace, ".opencode", "commands"), { recursive: true });
    await writeFile(
      join(workspace, ".opencode", "commands", "remove-me.md"),
      "---\nname: remove-me\ndescription: Remove command\n---",
      "utf8"
    );

    // Set up portable files in workspace directory
    await mkdir(join(workspace, ".opencode", "tools"), { recursive: true });
    await writeFile(join(workspace, ".opencode", "tools", "remove-me.ts"), "/* remove tool */", "utf8");
    ...
    await mkdir(join(workspace, ".opencode", "tools"), { recursive: true });
    await mkdir(join(workspace, ".opencode", "agents"), { recursive: true });
    await writeFile(join(globalSkills, "keep", "SKILL.md"), keepSkillContent, "utf8");
    await writeFile(join(globalSkills, "remove-me", "SKILL.md"), "legacy skill\n", "utf8");
    ...
    // onmyagent agent is auto-created during workspace initialization
    await writeFile(join(workspace, ".opencode", "agents", "onmyagent.md"), "onmyagent agent\n", "utf8");
    ...
    try {
      ...
      const response = await requestWorkspaceImportWithPreview(baseUrl, headers, {
        ...
        files: [
          { path: ".opencode/tools/shared.ts", content: "shared tool\n" },
          { path: ".opencode/agents/onmyagent.md", content: "onmyagent agent\n" },
        ],
      });
      ...
      expect(await pathExists(join(globalSkills, "remove-me"))).toBe(false);
      ...
      expect(await readFile(join(globalSkills, "keep", "SKILL.md"), "utf8")).toBe(keepSkillContent);
      ...
      expect(await readFile(auditLogPath("workspace"), "utf8")).toContain("Imported workspace config (update 1, remove 3)");
    } finally {
      server.stop(true);
      if (originalDataDir === undefined) {
        delete process.env.ONMYAGENT_DATA_DIR;
  ...
  test("replace import route honors empty sections", async () => {
    ...
    try {
      ...
      const response = await requestWorkspaceImportWithPreview(baseUrl, headers, {
        ...
        skills: [],
        commands: [],
        files: [{ path: ".opencode/agents/onmyagent.md", content: "onmyagent agent\n" }],
      });
  ...
  test("import route revalidates the preview after approval", async () => {
    ...
    try {
      ...
      const approvalResponse = await fetch(`${baseUrl}/approvals/${approvalId}`, {
        method: "POST",
        headers: {
          "X-OnMyAgent-Host-Token": "host-token",
          "Content-Type": "application/json",
        },
  ...
  test("replace import keeps existing items when an incoming write fails", async () => {
    ...
    await mkdir(join(globalSkills, "old"), { recursive: true });
    await writeFile(join(globalSkills, "old", "SKILL.md"), "old skill\n", "utf8");
    await writeFile(join(globalSkills, "new"), "blocks new skill directory\n", "utf8");
    ...
    try {
      ...
      expect(await readFile(join(globalSkills, "old", "SKILL.md"), "utf8")).toBe("old skill\n");
      expect(await readFile(join(globalSkills, "new"), "utf8")).toBe(
