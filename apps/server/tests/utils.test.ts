...
import { hashToken, shortId, parseList, ensureDir, exists } from "../src/core/utils.js";
...
describe("ensureDir + exists", () => {
  test("creates nested directory and reports it exists", async () => {
    const dir = join(tmpdir(), `onmyagent-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
