...
import { resolveOpencodeDbPath, seedOpencodeSessionMessages } from "../src/services/opencode-db.js";

async function createDb(): Promise<{ path: string; dispose: () => void }> {
  const dir = await mkdtemp(join(tmpdir(), "onmyagent-opencode-db-"));
...
describe("resolveOpencodeDbPath", () => {
  test("prefers an existing XDG opencode.db when present", async () => {
    const xdg = await mkdtemp(join(tmpdir(), "onmyagent-opencode-xdg-"));
  ...
  test("finds orchestrator-managed OpenCode dbs under ONMYAGENT_DATA_DIR", async () => {
    const root = await mkdtemp(join(tmpdir(), "onmyagent-orchestrator-data-"));
    const dir = join(root, "onmyagent-dev-data", "xdg", "data", "opencode");
    const file = join(dir, "opencode.db");
    await mkdir(dir, { recursive: true });
    await writeFile(file, "", "utf8");

    const previousDataDir = process.env.ONMYAGENT_DATA_DIR;
    const previousXdg = process.env.XDG_DATA_HOME;
    const previousChannel = process.env.OPENCODE_CHANNEL;
    const previousDb = process.env.OPENCODE_DB;
    try {
      process.env.ONMYAGENT_DATA_DIR = root;
      ...
      if (previousDataDir === undefined) delete process.env.ONMYAGENT_DATA_DIR;
      else process.env.ONMYAGENT_DATA_DIR = previousDataDir;
      if (previousXdg === undefined) delete process.env.XDG_DATA_HOME;
      else process.env.XDG_DATA_HOME = previousXdg;
      if (previousChannel === undefined) delete process.env.OPENCODE_CHANNEL;
      else process.env.OPENCODE_CHANNEL = previousChannel;
  ...
  test("finds legacy orchestrator-managed OpenCode dbs under ONMYAGENT_DATA_DIR", async () => {
    const root = await mkdtemp(join(tmpdir(), "onmyagent-orchestrator-data-"));
