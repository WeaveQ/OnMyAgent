...
import { ReloadEventStore } from "../src/services/events.js";
import { startReloadWatchers } from "../src/reload-watcher.js";
...
async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "onmyagent-reload-watcher-"));
  try {
    await fn(root);
  } finally {
  ...
  test("suppresses internal workspace bootstrap writes after refreshing the baseline", async () => {
    await withWorkspace(async (root) => {
      ...
      try {
        ...
        expect(ensured.reloadReasons.sort()).toEqual(["agents", "commands", "config"]);
        expect(events.list(workspace.id)).toEqual([]);
      } finally {
        watcher.close();
      }
