...
import { listPortableFiles, planPortableFiles, writePortableFiles } from "../src/workspace/portable-files.js";
...
async function makeWorkspace(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "onmyagent-portable-files-"));
  tempDirs.push(dir);
...
describe("portable files", () => {
  test("lists only extra shareable .opencode files", async () => {
    ...
    await writeFile(join(workspaceRoot, ".opencode", "onmyagent.json"), '{"version":1}\n', "utf8");
    ...
    expect(files).toEqual([
      { path: ".opencode/agents/onmyagent.md", content: "# agent\n" },
  ...
  test("rejects non-allowlisted portable files and path traversal", async () => {
    ...
    expect(() =>
      planPortableFiles(workspaceRoot, [{ path: ".opencode/onmyagent.json", content: "{}" }]),
    ).toThrow(/not allowed/i);
