...
describe("env-file", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "onmyagent-env-"));
  ...
  test("isReservedEnvKey blocks ONMYAGENT_ / OPENCODE_ prefixes", () => {
    expect(isReservedEnvKey("ONMYAGENT_TOKEN")).toBe(true);
  ...
  test("upsertMany rejects reserved keys", async () => {
    const svc = new EnvService({ path });
    const promise = svc.upsertMany([{ key: "ONMYAGENT_TOKEN", value: "x" }]);
    await expect(promise).rejects.toBeInstanceOf(InvalidEnvKeyError);
  ...
  test("readForInjection strips reserved keys even if present on disk", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        updatedAt: Date.now(),
        variables: [
          { key: "ONMYAGENT_TOKEN", value: "stolen" },
