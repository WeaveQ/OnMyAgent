...
// Directories that are managed separately and should be excluded from portable file scans
const EXCLUDED_PORTABLE_DIRS = new Set([".opencode/commands/", ".opencode/skills/"]);
...
async function walkPortableFiles(root: string, currentPath: string, output: PortableFile[]): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      // Skip excluded directories (managed separately)
      const relativeFromRoot = normalizePortablePath(absolutePath.slice(root.length + 1));
      if (EXCLUDED_PORTABLE_DIRS.has(relativeFromRoot + "/")) continue;
