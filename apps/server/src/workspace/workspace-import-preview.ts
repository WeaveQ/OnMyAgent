...
import { buildSkillContent } from "../services/skills.js";
import { exists } from "../core/utils.js";
import { sanitizeCommandName, validateCommandName, validateSkillName } from "../core/validators.js";
import {
  opencodeConfigPath,
  onmyagentConfigPath,
  projectCommandsDir,
...
export type WorkspaceImportChangeKind = "opencode" | "onmyagent" | "skill" | "command" | "file";
...
type WorkspaceImportSection = "opencode" | "onmyagent" | "skills" | "commands" | "files";
...
function normalizeModes(value: unknown): Record<string, WorkspaceImportMode> {
  ...
  return {
    ...
    onmyagent: readMode(record.onmyagent),
...
export function normalizeWorkspaceImportPayload(
  ...
  return {
    ...
    sections: {
      ...
      onmyagent: payload.onmyagent !== undefined,
...
async function listProjectSkillNames(_workspaceRoot: string): Promise<string[]> {
...
export async function buildWorkspaceImportPreview(
  ...
  if (input.onmyagent !== undefined) {
    ...
    changes.push({
      kind: "onmyagent",
      action: actionForTarget(existsBefore, !sameJson(before, after), input.modes.onmyagent),
      label: "OnMyAgent config",
      path: rel(workspaceRoot, path),
      absolutePath: path,
  ...
  if (input.sections.skills) {
    ...
    for (const skill of input.skills) {
      ...
      const path = join(globalSkillsDir(), skill.name, "SKILL.md");
    ...
    if (input.modes.skills === "replace") {
      for (const name of existing) {
        ...
        const path = join(globalSkillsDir(), name);
