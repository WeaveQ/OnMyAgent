...
// them into every spawned child (OpenCode, OnMyAgent server, opencode-router).
...
// is for service credentials, not OnMyAgent/OpenCode runtime knobs; users who
...
// identity.
const RESERVED_PREFIXES = ["ONMYAGENT_", "OPENCODE_"] as const;
...
export class EnvService {
  ...
  // Used by the Electron + orchestrator shells at spawn time.
  ...
  constructor(key: string, code: "invalid_env_key" | "reserved_env_key") {
    super(
      code === "reserved_env_key"
        ? `Environment variable name is reserved for OnMyAgent internals: ${key}`
