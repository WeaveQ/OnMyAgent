...
import {
  ...
  type SessionArchiveStorePool,
} from "./session-archive-store-pool.js";
import type { SessionArchiveRuntimePaths } from "./session-archive-sync.js";

/**
 * Lifecycle status for Studio. Uses the shared archive store pool so HTTP
 * routes and lifecycle probes share one SQLite handle (no bare open/close).
 */
export async function getSessionArchiveLifecycleStatus(input: {
  paths: SessionArchiveRuntimePaths;
  startedAt: number;
  version: string;
  /** Inject pool in tests; production uses the process default pool. */
  pool?: SessionArchiveStorePool;
}): Promise<SessionArchiveLifecycleStatus> {
  return withSessionArchiveStore(
    { dbPath: input.paths.dbPath, pool: input.pool },
    async (store) => {
      ...
      return {
        ...
        update: {
          ...
          blocker:
            "Session archive standalone updater is not used in Studio; Studio desktop updates are handled by the Studio application lifecycle.",
        },
        logs: {
          root: logRoot,
...
async function fileInfo(
  path: string,
): Promise<{ exists: boolean; bytes: number; modified_at: string | null }> {
  try {
...
async function logFileSummaries(
  root: string,
): Promise<
  Array<{ name: string; path: string; bytes: number; modified_at: string | null }>
> {
  let names: string[];
  try {
