...
import type { HubSkillItem } from "@onmyagent/types/server";
import { ApiError } from "../core/errors.js";
import { parseFrontmatter } from "../core/frontmatter.js";
import { exists } from "../core/utils.js";
import { validateSkillName } from "../core/validators.js";
...
export async function installHubSkill(
  ...
  for (const file of files) {
    ...
    const res = await fetch(`${rawBase}/${file.path}`, {
      headers: { "User-Agent": "onmyagent-server" },
    });
    if (!res.ok) {
      const text = await res.text().catch(() => "");
