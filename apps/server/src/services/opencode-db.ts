...
const DEFAULT_AGENT = "onmyagent";
const DEFAULT_PROVIDER = "openai";
const DEFAULT_MODEL = "gpt-5.4";
const ONMYAGENT_DEV_DATA_DIRS = ["onmyagent-dev-data", "opencode-dev"];
...
function opencodeOrchestratorDataDirs(): string[] {
  const root = process.env.ONMYAGENT_DATA_DIR?.trim();
  if (!root) return [];
  ...
  for (const name of ONMYAGENT_DEV_DATA_DIRS) {
...
export function seedOpencodeSessionMessages(input: {
  ...
  try {
    const run = db.transaction(() => {
      ...
      const updateSession = db.query("update session set time_updated = ?2 where id = ?1");
