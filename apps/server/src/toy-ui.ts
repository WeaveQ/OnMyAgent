...
<html lang="en">
  <head>
    ...
    <title>OnMyAgent Toy UI</title>
    <link rel="icon" type="image/svg+xml" href="/ui/assets/onmyagent-mark.svg" />
  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div class="title">
          <h1>OnMyAgent Toy UI</h1>
          <div class="sub">Local-first host contract harness (served by onmyagent-server)</div>
        </div>
        <div class="row">
          ...
          <div class="body">
            ...
            <div class="panel hidden" data-panel="skills">
              <div class="row">
                ...
                <span class="small">Managed in <span class="mono">~/.config/opencode/skills/</span></span>
              </div>
            ...
            <div class="panel hidden" data-panel="config">
              ...
              <div class="small">Uploads go to <span class="mono">.opencode/onmyagent/inbox/</span> inside the workspace.</div>

              <div class="hr"></div>

              <div class="row">
                <button class="btn" id="btn-artifacts">List artifacts</button>
                <span class="small">Downloads read from <span class="mono">.opencode/onmyagent/outbox/</span>.</span>
              </div>
...
const STORAGE_TOKEN = "onmyagent.toy.token";
const STORAGE_SESSION_PREFIX = "onmyagent.toy.session.";
...
async function main() {
  ...
  qs("#btn-skill").onclick = () => {
    const template = [
      ...
      "Requirements:",
      "- Skill name: my-skill",
      "- Write to ~/.config/opencode/skills/my-skill/SKILL.md",
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's OnMyAgent server config?")) return;
    try {
