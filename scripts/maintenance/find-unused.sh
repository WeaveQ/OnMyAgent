...
# ── Config ──────────────────────────────────────────────────────────────────
ONMYAGENT_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"

# Detect whether we're inside a factory layout (../../.. has _repos/)
FACTORY_CANDIDATE="$(cd "$ONMYAGENT_ROOT/../../.." 2>/dev/null && pwd)"
...
if [ -n "$FACTORY_ROOT" ]; then
  for d in "$FACTORY_ROOT"/_repos/*/; do
    [ -d "$d" ] || continue
    [ "$(cd "$d" && pwd)" = "$ONMYAGENT_ROOT" ] && continue
    SIBLING_REPOS+=("$d")
  done
fi

# ── Internal infra: all build/config/CI files that may reference source ────
# These are files WITHIN onmyagent that knip can't trace but that use source files
...
collect_infra_files() {
  ...
  for glob_pattern in "${INFRA_GLOBS[@]}"; do
    ...
    local matched
    matched=$(find "$ONMYAGENT_ROOT" -path "$ONMYAGENT_ROOT/$glob_pattern" 2>/dev/null || true)
    if [ -n "$matched" ]; then
      files="${files}${matched}"$'\n'
    fi
  done
  ...
  local pkg_jsons
  pkg_jsons=$(find "$ONMYAGENT_ROOT" -name package.json -not -path '*/node_modules/*' -not -path '*/.git/*')
  files="${files}${pkg_jsons}"$'\n'
  # And all tsconfig*.json files (for path aliases / includes)
  local tsconfigs
  tsconfigs=$(find "$ONMYAGENT_ROOT" -name 'tsconfig*.json' -not -path '*/node_modules/*' -not -path '*/.git/*')
...
# Search sibling repo CI/CD and build scripts for an onmyagent-relative path.
# Only checks infra files (workflows, Dockerfiles, build scripts), NOT source code,
# since no sibling repo has an npm dependency on onmyagent packages.
search_sibling_ci() {
  local pattern="$1"
  local hits=""
...
format_refs() {
  ...
  while IFS= read -r ref; do
    ...
    local short="$ref"
    if [[ "$ref" == "$ONMYAGENT_ROOT/"* ]]; then
      short="${ref#"$ONMYAGENT_ROOT/"}"
...
for filepath in "${UNUSED_FILES[@]}"; do
  ...
  # Only search by the onmyagent-relative path (precise) or unique filename.
  # Since no sibling repo has npm deps on onmyagent packages, we only check
...
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo -e "${DIM}  No imports in onmyagent source, no references in CI/CD, build${RESET}"
