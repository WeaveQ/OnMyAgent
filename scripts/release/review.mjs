...
const versions = {
  ...
  orchestratorOnMyAgentServerRange:
    orchestratorPkg.dependencies?.["onmyagent-server"] ?? null,
};
...
addCheck(
  "App/onmyagent-orchestrator versions match",
  versions.app &&
...
addCheck(
  "App/onmyagent-server versions match",
...
const onmyagentServerRange = versions.orchestratorOnMyAgentServerRange ?? "";
const onmyagentServerPinned = /^\d+\.\d+\.\d+/.test(onmyagentServerRange);
if (!onmyagentServerRange) {
  addWarning("onmyagent-orchestrator is missing an onmyagent-server dependency.");
} else if (!onmyagentServerPinned) {
  addWarning(
    `onmyagent-orchestrator onmyagent-server dependency is not pinned (${onmyagentServerRange}).`,
  );
} else {
  addCheck(
    "OnMyAgent server dependency matches server version",
    versions.server && onmyagentServerRange === versions.server,
    `${onmyagentServerRange} vs ${versions.server ?? "?"}`,
  );
}
...
const sidecarManifestPath = resolve(
  ...
  "dist",
  "sidecars",
  "onmyagent-orchestrator-sidecars.json",
);
if (existsSync(sidecarManifestPath)) {
  const manifest = readJson(sidecarManifestPath);
  addCheck(
    "Sidecar manifest version matches onmyagent-orchestrator",
    versions.orchestrator && manifest.version === versions.orchestrator,
    `${manifest.version ?? "?"} vs ${versions.orchestrator ?? "?"}`,
  );
  const serverEntry = manifest.entries?.["onmyagent-server"]?.version;
  if (serverEntry) {
    addCheck(
      "Sidecar manifest onmyagent-server version matches",
  ...
  addWarning(
    "Sidecar manifest missing (run pnpm --filter onmyagent-orchestrator build:sidecars).",
  );
}
