#!/usr/bin/env bash
#
# onmyagent-debug.sh — one-stop observability + lifecycle control for the
# OnMyAgent dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan onmyagent/opencode processes (ppid == launchd)
...
#   wait-healthy    block until onmyagent-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Electron shell + CDP on 127.0.0.1:9823 for browser automation.
# Sidecars run from apps/desktop/resources/sidecars/*.
#
# Teardown ordering (important, both variants):
#   1. pnpm dev supervisor                 (parent supervisor)
#   2. Electron main+helpers (node_modules/electron/...Electron.app)
#   3. Vite                (node node_modules/.../vite)
#   4. orchestrator + onmyagent-server + opencode
#
...
#   - ~/Library/Application Support/com.differentai.onmyagent.dev/** (tokens,
#     workspaces registry, prefs).
#   - /Applications/OnMyAgent.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${ONMYAGENT_DEV_LOG_FILE:-$HOME/.onmyagent/debug/onmyagent-dev.log}"
PNPM_DEV_LOG="${ONMYAGENT_PNPM_DEV_LOG:-/tmp/onmyagent-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${ONMYAGENT_PNPM_DEV_PID:-/tmp/onmyagent-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${ONMYAGENT_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${ONMYAGENT_ELECTRON_REMOTE_DEBUG_PORT:-9823}"

# ---------------------------------------------------------------------------
# Helpers

log() { printf '[onmyagent-debug] %s\n' "$*"; }

kill_by_pattern() {
  # Sends TERM then KILL to every process whose full command line matches the
  # given regex. Used for targeted teardown of development processes.
  local pattern="$1"
  local pids
...
discover_onmyagent_server_port() {
  ps -Ao command \
    | grep -E "apps/desktop/resources/sidecars/onmyagent-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.differentai\.onmyagent/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.ONMYAGENT_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/node_modules\/electron\/dist\/Electron\.app\/Contents\/MacOS\/Electron|apps\/desktop\/scripts\/electron-dev\.mjs|apps\/desktop\/resources\/sidecars\/onmyagent-server|apps\/desktop\/resources\/sidecars\/onmyagent-orchestrator|apps\/desktop\/resources\/sidecars\/opencode( |\/)|vite|pnpm .*dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== onmyagent-server ==="
  local port
  port=$(discover_onmyagent_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev onmyagent-server running)"
  else
  ...
  oc_port=$(ps -Ao command \
    | grep -E "apps/desktop/resources/sidecars/onmyagent-orchestrator" \
    | grep -v grep \
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /onmyagent-server|onmyagent-orchestrator|opencode( |\/)/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with ONMYAGENT_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /onmyagent-server|onmyagent-orchestrator|opencode( |\/)/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(ONMYAGENT_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_onmyagent_server_port)
  if [[ -n "$port" ]]; then
    echo "  onmyagent-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no onmyagent-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/onmyagent-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set ONMYAGENT_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  # 5. onmyagent-server / orchestrator / opencode for the
  #    current dev build. These are the longest-lived children and the ones
  #    most likely to orphan after an unclean shutdown.
  #    Electron dev runs them from resources/sidecars/.
  kill_by_pattern "apps/desktop/resources/sidecars/onmyagent-server"
  kill_by_pattern "apps/desktop/resources/sidecars/onmyagent-orchestrator"
...
start() {
  ...
  cd "$REPO_ROOT"
  local pid
  log "starting pnpm dev (log sink: $DEV_LOG_FILE, CDP: 127.0.0.1:9823)"
  env ONMYAGENT_DEV_LOG_FILE="$DEV_LOG_FILE" \
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "onmyagent-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "onmyagent-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset() {
  ...
  log "http://127.0.0.1:9823; browser automation can attach there."
