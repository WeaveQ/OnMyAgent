 ...
 *
 * Usage:
 *   node scripts/checks/i18n-audit.mjs              # full audit (default, excludes --hardcoded, --prune, --sort)
 *   node scripts/checks/i18n-audit.mjs --ci         # same as default but does not fail on missing non-en keys
 *   node scripts/checks/i18n-audit.mjs --missing    # missing keys (in EN but not in locale)
 *   node scripts/checks/i18n-audit.mjs --orphan     # orphan keys (in locale but not in EN)
 *   node scripts/checks/i18n-audit.mjs --duplicates # duplicate keys in any locale
 *   node scripts/checks/i18n-audit.mjs --unused     # unused keys (in EN but not referenced in repo)
 *   node scripts/checks/i18n-audit.mjs --dangling   # t() calls referencing keys not in en.ts
 *   node scripts/checks/i18n-audit.mjs --aliases    # aliased t() calls (translate/tr instead of t)
 *   node scripts/checks/i18n-audit.mjs --placeholders # placeholder integrity check
 *   node scripts/checks/i18n-audit.mjs --plurals    # each locale has bare key OR all CLDR plural forms it needs
 *   node scripts/checks/i18n-audit.mjs --hardcoded  # hardcoded English strings in source files
 *   node scripts/checks/i18n-audit.mjs --prune      # (destructive) remove unused keys from all locales
 *   node scripts/checks/i18n-audit.mjs --sort       # (destructive) alphabetically sort keys in all locales
 */
...
/** Resolve either a legacy single-file locale or a split locale directory. */
function resolveLocalePath(filePath) {
  if (existsSync(filePath)) {
    const content = readFileSync(filePath, "utf-8");
    const wrapperMatch = content.match(/export \{ default \} from "\.\/([^"]+)";/);
    if (wrapperMatch) return join(dirname(filePath), `${wrapperMatch[1]}.ts`);
    return filePath;
  }

  const indexFile = join(filePath.replace(/\.ts$/, ""), "index.ts");
...
function parseLocale(filePath) {
  const resolvedPath = resolveLocalePath(filePath);
  const content = readFileSync(resolvedPath, "utf-8").replace(
    /import \{ APP_NAME \} from "\.\.\/brand";\n?|import \{ APP_NAME \} from "\.\/brand";\n?/g,
    'const APP_NAME = "OnMyAgent";\n',
  );
  const match = content.match(/export default \{([\s\S]*?)\} as const;/);
  if (!match) throw new Error(`Could not parse ${resolvedPath}`);

  const namespaceImports = [...content.matchAll(/^import\s+([a-zA-Z0-9_]+)\s+from\s+"\.\/([^"]+)";/gm)];
  if (namespaceImports.length > 0) {
    const namespaceObjects = Object.fromEntries(
      namespaceImports.map(([, variableName, importPath]) => [
        variableName,
        parseLocale(join(dirname(resolvedPath), `${importPath}.ts`)),
      ]),
    );
    return new Function(
      "namespaces",
      `const APP_NAME = "OnMyAgent"; const { ${namespaceImports.map(([, name]) => name).join(", ")} } = namespaces; return {${match[1]}}`,
    )(namespaceObjects);
  }

  return new Function(`const APP_NAME = "OnMyAgent"; return {${match[1]}}`)();
}
...
function localeSourceFiles(filePath) {
  const resolvedPath = resolveLocalePath(filePath);
  if (!resolvedPath.endsWith("/index.ts")) return [resolvedPath];
  return collectSourceFiles(dirname(resolvedPath), (dir) => false)
    .filter((file) => basename(file) !== "index.ts")
    .sort();
}

/** Find duplicate keys in a locale (must use regex — JS object eval dedupes silently). */
function findDuplicates(filePath) {
  const seen = new Map();
  const dupes = [];
  for (const sourceFile of localeSourceFiles(filePath)) {
    const content = readFileSync(sourceFile, "utf-8");
    for (const match of content.matchAll(/^\s*"([^"]+)"\s*:/gm)) {
      const key = match[1];
      if (seen.has(key)) dupes.push(key);
      else seen.set(key, sourceFile);
    }
  }
  return dupes;
}
...
// Locales without plurals (e.g. Chinese) use the bare key while en
...
if (shouldRun("--hardcoded")) {
  ...
  const excludePatterns = [
    /import\b/, /from\s+"/, /class=/, /className=/, /\btype\s/, /\bconst\s/,
    /variant=/, /\bt\(/, /translate\(/, /new Error\(/, /throw new /,
    /console\./, /event\.key/, /event\.code/, /headers?\./, /"Authorization"/,
    /"Content-Type"/, /"Notification"/, /"Backspace"/, /"Escape"/, /"Arrow/,
    /"Enter"/, /"Process"/, /"object"/, /"string"/, /"number"/, /"boolean"/,
    /"unknown"/, /"local"/, /"remote"/, /"session"/, /"assistant"/, /"user"/,
    /Promise</, /=>\s*Promise/, /^\s*[A-Za-z0-9_]+\??:\s*\(/,
    /^\s*\/\//, /^\s*\/\*/, /^\s*\*/,
  ];

  const allowedLiteralPattern = /"(?:Windows|Android|Linux|Python|Telegram|Slack|Excel|Notion|Codex|Hermes|Gemini|Claude|Gmail|Supabase|Cloudflare|Vercel|OpenCode|OpenPackage|OnMyAgent|Plugin|Skill|MCPs|Browser|Anthropic|Google|Mistral|Fireworks|Perplexity|English|Chinese|OnMyAgent)"/;
  const uiAttributePattern = /\b(?:title|aria-label|placeholder|label|description|primaryCta|message|error|statusText):?=?(?:\{|\s*)"[A-Z][^"`{}]*(?:[a-z][^"`{}]*)"/;
  const jsxTextPattern = /<[A-ZA-Za-z][^>]*>\s*[A-Z][^<>{}`]*(?:[a-z][^<>{}`]*)\s*<\/[^>]+>/;
  const objectUiPattern = /\b(?:title|label|description|primaryCta|message|error|statusText):\s*"[A-Z][^"`{}]*(?:[a-z][^"`{}]*)"/;
  const englishPattern = new RegExp([
    uiAttributePattern.source,
    jsxTextPattern.source,
    objectUiPattern.source,
  ].join("|"));
  ...
  for (const full of hardcodedFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/=>\s*Promise<|\)\s*=>\s*Promise<|:\s*\([^)]*\)\s*=>/.test(line)) continue;
      if (allowedLiteralPattern.test(line)) continue;
