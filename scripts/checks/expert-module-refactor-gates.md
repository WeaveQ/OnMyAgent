...
`pnpm task check types` in a detached temporary worktree. A missing base on a
`pull_request` event is a typed `BASE_UNAVAILABLE` error. A `push` whose
`event.before` object is gone (force-push / rewritten history) checks the new
tip only. An oversized range is `RANGE_TOO_LARGE`.
