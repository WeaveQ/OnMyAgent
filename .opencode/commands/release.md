---
description: Run the OnMyAgent release flow
---

You are running the OnMyAgent release flow in this repo.
...
7. If releasing onmyagent-orchestrator sidecars, build deterministically with `SOURCE_DATE_EPOCH`, upload assets to `onmyagent-orchestrator-vX.Y.Z`, and publish `onmyagent-orchestrator`.
8. If `onmyagent-server` or `opencode-router` versions changed, publish those packages.
