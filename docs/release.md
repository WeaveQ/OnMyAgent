...
GitHub macOS builds are signed with `Developer ID Application: li yingkun (WP5S7KB7R7)`. Notarization is still **off** until notary API secrets exist, so ship GitHub builds as **developer preview** only:
...
> automatically. The full Apple identity is `Developer ID Application: li yingkun (WP5S7KB7R7)`.

SHA-1: `0A844944FB3F93D66D2F5F24D945ADC7AB7ED392`.
